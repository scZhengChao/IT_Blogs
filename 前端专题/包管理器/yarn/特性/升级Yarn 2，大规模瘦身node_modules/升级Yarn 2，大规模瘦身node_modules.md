# 升级Yarn 2，大规模瘦身node\_modules

## 目录

- [安装设置](#安装设置)
- [解决错误](#解决错误)
- [与vscode中的eslint整合](#与vscode中的eslint整合)
- [与storybook整合](#与storybook整合)
...
嗯，目前`yarn`的版本号是`1.22.11`，那我们如何安装`yarn 2`呢？
...
然后我们来看一下项目文件夹下多了这么几个文件，首**先就是根目录下多了一个`.yarnrc.yml`(.yarnrc)，里面只有一句话：**
...
```typescript 
Yarn 2可能导致vscode中的eslint失效，因此需要在项目根目录下执行以下命令：
```


同时，安装对`eslint-import-resolver-node`的依赖：
...
同时，`eslint-import-resolver-alias`依赖年久失修，会导致更严重的问题，因此建议拆除并关闭相应警告。
...
如果在使用`storybook`时有错误，那么目前的`berry`版本还不够，需要将`yar``n`升级到更高的`canary`版本：
