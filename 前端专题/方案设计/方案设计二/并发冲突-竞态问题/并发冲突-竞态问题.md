...
利用[saga](<saga.md> "saga") 的 `takeLatest`

# 方案二

[处理竞态问题](处理竞态问题.md "处理竞态问题") 利用useEffect；react的快照特性 （这个方案只是抛弃了返回）
...
[使用 axios 拦截器解决「 前端并发冲突 」 问题](<使用 axios 拦截器解决「 前端并发冲突 」 问题.md> "使用 axios 拦截器解决「 前端并发冲突 」 问题") ；统一封装取消请求函数调用
