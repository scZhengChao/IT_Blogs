...
- [if 表达式](#if表达式)
  - [使用 else if 处理多重条件](#使用else-if处理多重条件)
  - [在 let 语句中使用 if](#在let语句中使用if)
- [使用循环重复执行](#使用循环重复执行)
  - [使用 loop 重复执行代码](#使用loop重复执行代码)
  - [从循环返回值](#从循环返回值)
  - [循环标签：在多个循环之间消除歧义](#循环标签在多个循环之间消除歧义)
  - [while 条件循环](#while条件循环)
  - [使用 for 遍历集合](#使用for遍历集合)
- [总结](#总结)

根据条件是否为真来决定是否执行某些代码，以及根据条件是否为真来重复运行一段代码的能力是大部分编程语言的基本组成部分。Rust 代码中最常见的用来控制执行流的结构是 `if` 表达式和循环。

### [if](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#if-表达式 "if")[ 表达式](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#if-表达式 " 表达式")
...
所有的 `if` 表达式都以 `if` 关键字开头，其后跟一个条件。在这个例子中，条件检查变量 `number` 的值是否小于 5。在条件为 `true` 时希望执行的代码块位于紧跟条件之后的大括号中。`if` 表达式中与条件关联的代码块有时被叫做 *arms*，就像第二章 [“比较猜测的数字和秘密数字”](https://kaisery.github.io/trpl-zh-cn/ch02-00-guessing-game-tutorial.html#比较猜测的数字和秘密数字 "“比较猜测的数字和秘密数字”") 部分中讨论到的 `match` 表达式中的分支一样。
...
#### [使用 ](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-else-if-处理多重条件 "使用 ")[else if](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-else-if-处理多重条件 "else if")[ 处理多重条件](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-else-if-处理多重条件 " 处理多重条件")
...
#### [在 ](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#在-let-语句中使用-if "在 ")[let](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#在-let-语句中使用-if "let")[ 语句中使用 ](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#在-let-语句中使用-if " 语句中使用 ")[if](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#在-let-语句中使用-if "if")
...
### [使用循环重复执行](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用循环重复执行 "使用循环重复执行")
...
#### [使用 ](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-loop-重复执行代码 "使用 ")[loop](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-loop-重复执行代码 "loop")[ 重复执行代码](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-loop-重复执行代码 " 重复执行代码")
...
幸运的是，Rust 提供了一种从代码中跳出循环的方法。可以使用 `break` 关键字来告诉程序何时停止循环。回忆一下在第二章猜猜看游戏的 [“猜测正确后退出”](https://kaisery.github.io/trpl-zh-cn/ch02-00-guessing-game-tutorial.html#猜测正确后退出 "“猜测正确后退出”") 部分使用过它来在用户猜对数字赢得游戏后退出程序。

我们在猜谜游戏中也使用了 `continue`。循环中的 `continue` 关键字告诉程序跳过这个循环迭代中的任何剩余代码，并转到下一个迭代。

#### [从循环返回值](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#从循环返回值 "从循环返回值")
...
#### [循环标签：在多个循环之间消除歧义](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#循环标签在多个循环之间消除歧义 "循环标签：在多个循环之间消除歧义")
...
#### [while](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#while-条件循环 "while")[ 条件循环](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#while-条件循环 " 条件循环")
...
#### [使用 ](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-for-遍历集合 "使用 ")[for](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-for-遍历集合 "for")[ 遍历集合](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#使用-for-遍历集合 " 遍历集合")
...
## [总结](https://kaisery.github.io/trpl-zh-cn/ch03-05-control-flow.html#总结 "总结")
