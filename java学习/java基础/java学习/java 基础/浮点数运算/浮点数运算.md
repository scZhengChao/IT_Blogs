...
浮点数运算和整数运算相**比，只能进行加减乘除这些数值计算，不能做位运算和移位运算。**
...
**浮点数在内存的表示方法和整数比更加复杂**。**Java的浮点数完全遵循**[**IEEE-754**](https://standards.ieee.org/ieee/754/6210/ "IEEE-754")**标准**，这也是绝大多数计算机平台都支持的浮点数标准表示方法。
...
可以将浮点数强制转型为整数。在转型时，**浮点数的小数部分会被丢掉**。如果转型后超过了**整型能表示的最大范围，将返回整型的最大值。例如：**
