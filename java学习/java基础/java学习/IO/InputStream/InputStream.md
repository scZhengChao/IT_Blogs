...
`InputStream`就是Java标准库提供的**最基本的输入流**。它位于`java.io`这个包里。`java.io`包提供了所有同步IO的功能。
...
这个方法会读**取输入流的下一个字节，并返回字节表示的`int`值（0\~255）**。如果\*\*已读到末尾，返回`-1`\*\***表示不能继续读取了。**
...
我们还要注意到在读取或写入IO流的过程\*\*中，可能会发生错误，例如，文件不存在导致无法读取，没有写权限导致写入失败，等等，这些底层错误由Java虚拟机自动封装成`IOException`\*\***异常并抛出。因此，所有与IO操作相关的代码都必须正确处理**`IOException`。
...
利用上述方法一次读取多个字节时，需要先定义一个`byte[]`数组作为缓冲区，`read()`方法会**尽可能多地读取字节到缓冲区， 但不会超过缓冲区的大小。`read()`****方法的返回值不再是字节的****`int`值，而是返回实际读取了多少个字**节。如果返回`-1`，表示没有更多的数据了。
