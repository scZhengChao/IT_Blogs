...
而\*\*聚合操作则不一样，聚合操作会立刻促使`Stream`****输出它的每一个元素，并依次纳入计算，以获得最终结果。所以，对一个****`Stream`\*\***进行聚合操作，会触发一系列连锁反应：**
...
`reduce()`只是一种聚合操作，如果我们希望把`Stream`的元素保存到集合，例如`List`，因为`List`的元素是确定的Java对象，因此，把`Stream`变为`List`不是**一个转换操作，而是一个聚合操作，它会强制`Stream`输出每**个元素。
