...
我们放入的顺序是`"apple"`、`"pear"`、`"banana"`，但是取出的顺序却是`"apple"`、`"banana"`、`"pear"`，这是**因为从字符串的排序看，**`"apple"`排在最前面，`"pear"`排在最后面。

因此，放入`PriorityQueue`的元素，**必须实现**\*\*`Comparable`****接口**，`PriorityQueue`会根据**元素的排序顺序决定出队的优先级 \*\*。
