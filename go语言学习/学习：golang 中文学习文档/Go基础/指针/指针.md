...
关于指针有两个常用的操作符，**一个是取地址符**\*\*`&`****，另一个是解引用符****`*`**。对一个**变量进行取地址，会返回对应类型的指针 \*\*，例如：
...
解引用符则有两个用途，第一个**是访问指针所指向的元素**，也就是解引用，例如
...
`new`函数**只有一个参数那就是类型，并返回一个对应类型的指针**，函数会为**该指针分配内存，并且指针指向对应类型的零值**，例如：
...
> 提示
> 标准库`unsafe`提供了许多用于低级编程的操作，其中就包括指针运算，前往[标准库-unsafe](https://golang.xiniushu.com/语言入门/语法基础/src/语言入门/标准库/unsafe.html "标准库-unsafe")了解细节。
...
> 提示
> 切片可以看作是一种特殊的指针，前往[切片原理](https://golang.xiniushu.com/语言入门/语法基础/src/语言进阶/原理解析/slice.html "切片原理")了解更多细节。
