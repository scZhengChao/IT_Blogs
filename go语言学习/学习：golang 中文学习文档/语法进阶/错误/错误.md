...
### [#](https://golang.xiniushu.com/语言入门/语法进阶/95.error.html#创建 "#")创建
...
`target`必须是指向`error`的指针，由于在**创建结构体时返回的是结构体指针**，所以\*\*`error`****实际上****`*TimeError`****类型的，那么****`target`****就必须是****`**TimeError`\*\***类型的。**
