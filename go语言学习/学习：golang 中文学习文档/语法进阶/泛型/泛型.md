...
- [#泛型结构](#泛型结构)
  - [泛型结构注意点](#泛型结构注意点)
    - [泛型不能作为一个类型的基本类型](#泛型不能作为一个类型的基本类型)
    - [泛型类型无法使用类型断言](#泛型类型无法使用类型断言)
    - [匿名结构不支持泛型](#匿名结构不支持泛型)
    - [匿名函数不支持自定义泛型](#匿名函数不支持自定义泛型)
    - [不支持泛型方法](#不支持泛型方法)
- [类型集](#类型集)
  - [并集](#并集)
  - [交集](#交集)
  ...
  - [底层类型](#底层类型)
- [类型集注意点](#类型集注意点)
  - [带有方法集的接口无法并入类型集](#带有方法集的接口无法并入类型集)
    - [类型集无法当作类型实参使用](#类型集无法当作类型实参使用)
  - [类型集中的交集问题](#类型集中的交集问题)
  - [类型集不能直接或间接的并入自身](#类型集不能直接或间接的并入自身)

最初的Go是没有泛型这一说法的，但自从诞生以来，社区关于Go呼声最高的事情就是希望加入泛型。终于Go在1.18版本加入了对泛型的支持，不过Go的泛型依旧保留了Go自己的特点。

## [#](https://golang.xiniushu.com/语言入门/语法进阶/90.generic.html#示例 "#")示例
...
## [#](https://golang.xiniushu.com/语言入门/语法进阶/90.generic.html#泛型结构 "#")泛型结构
...
> An interface type defines a [**type set**](https://go.dev/ref/spec#Interface_types "type set")
