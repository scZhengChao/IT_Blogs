...
- [Cannot read property 'catch' of undefined](#Cannot-read-property-catch-of-undefined)
- [InvalidStateError: An attempt was made to use an object that is not, or is no longer, usable](#InvalidStateError-An-attempt-was-made-to-use-an-object-that-is-not-or-is-no-longer-usable)
- [NotAllowedError](#NotAllowedError)
- [iOS 中页面隐藏和显示时，播放audio行为异常](#iOS-中页面隐藏和显示时播放audio行为异常)

[   https://juejin.cn/post/6844903741838082062](https://juejin.cn/post/6844903741838082062 "   https://juejin.cn/post/6844903741838082062")
...
参考资料：[HTMLMediaElement.play() Returns a Promise](https://link.juejin.cn/?target=https://developers.google.com/web/updates/2016/03/play-returns-promise "HTMLMediaElement.play() Returns a Promise")

### InvalidStateError: An attempt was made to use an object that is not, or is no longer, usable

原因：对于还没有设置src的audio，就直接设置currentTime是会触发一个INVALID\_STATE\_ERR异常的。即使是设置currentTime = 0也会触发这个异常

解决：在设置currentTime之前，必须先设置audio的src

参考资料：[Offsets into the media resource](https://link.juejin.cn/?target=https://www.w3.org/TR/2011/WD-html5-20110113/video.html#offsets-into-the-media-resource "Offsets into the media resource")
...
这种情况，某些版本【在iOS12.0.1亲测有坑】也会触发`NotAllowedError`异常，应该避免这种情况，可以考虑如下hack手段解决
...
- 场景三：创建了多个audio元素，但是并不是每个audio都是通过用户点击来调用的play()的，这时候某些版本【在iOS12.0.1亲测有坑】也会触发`NotAllowedError`异常。对于这种情况，最好的办法就是**只创建一个audio元素，后面通过改变src来播放不同的**音乐资源。只要**audio通过了事件回调里调用过play，后续都可以直接调用play了，而无需再次绑定事件回调里去执行**，并且这样也可以避免创建多个audio来减少内存使用。
