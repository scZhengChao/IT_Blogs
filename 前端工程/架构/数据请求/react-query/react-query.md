...
[ React Query 使用手册 非常好用的query库，目的是为了缓存后端api的结果，不用像以前一样，手动将结果一个一个存储到store，并且提供了一些非常好用的hook方法 默认支持异步 它并不是用于替代axios等请求库，而只是作为外层的封装，方便控制请求与结果  安装配置1npm i @tanstack/react-query  配置就是将其作为一个provider注入到app中 12345678910const qu https://haofly.net/react-query/](https://haofly.net/react-query/ " React Query 使用手册 非常好用的query库，目的是为了缓存后端api的结果，不用像以前一样，手动将结果一个一个存储到store，并且提供了一些非常好用的hook方法 默认支持异步 它并不是用于替代axios等请求库，而只是作为外层的封装，方便控制请求与结果  安装配置1npm i @tanstack/react-query  配置就是将其作为一个provider注入到app中 12345678910const qu https://haofly.net/react-query/")

[   https://react-query-v2.tanstack.com/reference/useQuery](https://react-query-v2.tanstack.com/reference/useQuery "   https://react-query-v2.tanstack.com/reference/useQuery")
...
[   https://www.npmjs.com/package/react-query](https://www.npmjs.com/package/react-query "   https://www.npmjs.com/package/react-query")
