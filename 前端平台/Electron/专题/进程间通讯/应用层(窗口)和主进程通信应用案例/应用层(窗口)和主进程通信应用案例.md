...
[将ipcRender，process注入到应用层](将ipcRender，process注入到应用层.md "将ipcRender，process注入到应用层")

[应用层封装注入的Api](应用层封装注入的Api.md "应用层封装注入的Api")

[应用层发送事件到主进程](应用层发送事件到主进程.md "应用层发送事件到主进程")

[主进程接收事件并向应用层发送事件](主进程接收事件并向应用层发送事件.md "主进程接收事件并向应用层发送事件")
