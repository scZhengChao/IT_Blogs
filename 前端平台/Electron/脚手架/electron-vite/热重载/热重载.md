...
热重载是指在**主进程或预加载脚本模块发生变化时快速重新构建并重启** Electron 程序。事实上，并不是真正的热重载，而是类似的。它为开发者带来了很好的开发体验。
...
2. 使用配置项 `build.watch` 并设为 `{}`。此外，还可以配置更多的 watcher 选项，见 [WatcherOptions](https://rollupjs.org/guide/en/#watch-options "WatcherOptions")。
