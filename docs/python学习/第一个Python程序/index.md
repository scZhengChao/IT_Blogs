...
- [命令行模式](#命令行模式)
- [Python交互模式](#Python交互模式)
- [命令行模式和Python交互模式](#命令行模式和Python交互模式)
...
![](https://liaoxuefeng.com/books/python/first-program/type.jpg)
...
如果要让Python打印出指定的文字，可以用`print()`函数，然后**把希望打印的文字用单引号或者双引号**括起来，但不能混用单引号和双引号：
...
请注意**区分命令行模式和Python交互模式。**

在\*\*命令行模式下，****可以执行****`python`****进入****`Python`\*\***交互式环境**，也可以执行`python hello.py`运行一个`.py`文件。

**执行一个**`.py`**文件*****只能*****在命令行模式执行**。如果敲一个命令`python hello.py`，看到如下错误：

![](./image/image_NbPmjzonPr.png)

错误提示`No such file or directory`说明这个`hello.py`在当前目录下*找不到*，必须先把当前目录切换到`hello.py`所在的目录下，才能正常执行：

![](./image/image_Hp2RBEKXka.png)
...
最后，Python\*\*交互模式的代码是输入一行，执行一行，而命令行模式下直接运行`.py`\*\***文件是一次性执行该文件内的所有代码**。可见，Python交互模式主要是为了调试Python代码用的，也便于初学者学习，它*不是*正式运行Python代码的环境！
