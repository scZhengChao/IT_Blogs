...
![](./assets/image/image_dcfist7f0u.webp)
...
![](./assets/image/image_H-Zl-OBk-c.webp)

如果组件在 `useEffect` `useMemo` `useCallback` 中使用非原始值作为依赖项 `dependency` ，那也应该使用缓存

![](./assets/image/image_vmoCIiAIzE.webp)

## 避免 `Context` 提供的数据引起重复渲染

### ✅ 缓存 `Provider` 提供的数据

![](./assets/image/image_g1kLy9FgCX.webp)

### ✅ 将读取，写入数据分割成不同的 `Provider`

![](./assets/image/image__NmepE4QPY.webp)

### ✅ 将数据分割成小的 `Provider`

![](./assets/image/image_TOjZ0Ib6vP.webp)

[通过 Children 或 Props 传递元素](<./通过 Children 或 Props 传递元素/index.md> "通过 Children 或 Props 传递元素")

[使用 Memoization](<./使用 Memoization/index.md> "使用 Memoization")
