...
![](./assets/image/image_rSriq6v8IE.webp)
...
> 这些约定仅作用于**组件库文档，不会对组件库源码构建产生任何影响**，请勿基于**这些约定来实现特定的组件库功能（比如为组件库添加全局样式）。**
...
约定`.dumi/app.(js|jsx|ts|tsx)`为运行时配置文件，适用于修改框架[运行时配置](https://d.umijs.org/config/runtime "运行时配置")的场景。
...
约定`.dumi/favicon.(ico|gif|png|jpg|jpeg|svg|avif|webp)`为站点 favicon 图标，当存在任一后缀文件时，dumi 将会自动在 HTML 中插入对应的`link`标签以应用 favicon；你也可以通过[favicons](https://d.umijs.org/config#favicons "favicons")配置项来手动指定 favicon。
