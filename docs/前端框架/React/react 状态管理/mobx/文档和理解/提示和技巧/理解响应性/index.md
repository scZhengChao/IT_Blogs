...
- [实例](#实例)
  - [对象](#对象)
    - [正确的做法：在跟踪函数中使用某个对象的属性](#正确的做法在跟踪函数中使用某个对象的属性)
    - [不正确的做法：更改一个不可观察引用的值](#不正确的做法更改一个不可观察引用的值)
    - [不正确的做法：在跟踪函数外部使用可观察属性](#不正确的做法在跟踪函数外部使用可观察属性)
    - [正确的做法：在跟踪函数内部使用可观察属性](#正确的做法在跟踪函数内部使用可观察属性)
    - [不正确的做法：在不跟踪的情况下保留一个可观察对象的局部引用](#不正确的做法在不跟踪的情况下保留一个可观察对象的局部引用)
    - [常见陷阱：console.log](#常见陷阱consolelog)
  - [数组](#数组)
    ...
    - [正确的做法：使用尚不存在的字典的键](#正确的做法使用尚不存在的字典的键)
  - [异步](#异步)
    - [MobX 不跟踪异步访问的数据](#MobX-不跟踪异步访问的数据)
  - [proxy](#proxy)
    - [使用不可观察的对象属性](#使用不可观察的对象属性)
    - [\[无代理支持\] 不正确的做法：使用尚不存在的对象属性](#无代理支持-不正确的做法使用尚不存在的对象属性)
    - [\[无代理支持\] 正确的做法：使用 MobX 实用工具读/写对象](#无代理支持-正确的做法使用-MobX-实用工具读写对象)
    - [摘要](#摘要)
...
> Mobx 会对**跟踪函数执行时读取的**任何 *存在的* **可观察的 *****属性***** 做出响应。**

- “读取” 使用一个对象的属性, “读取”有两种方式。一种是点访问（dotting into），例如 [user.name](http://user.name "user.name") 。另一种是方括号访问，例如 user\['name']、todos\[3]。
...
- *跟踪函数执行时”* 这意味着只有那些在跟踪函数执行**时读取的可观察对象才会被跟踪**。这些值在跟踪函数中是直接使用还是间接使用并不重要 **。但是从函数“引发”出来的东西将不会被跟踪**（例如，`setTimeout` `promise.then`， `await` 等）。
...
你可以通过在跟踪函数中调用 [trace()](https://www.mobxjs.com/analyzing-reactivity "trace()") 来验证 MobX 进行了跟踪，在上面函数所示的情况下，它将输出以下内容：
...
这 **不会** 使 MobX 响应。`message` 发生了改变，但是 `message` **本身并不是可观察的，它仅仅是一个 *****指向***** 一个可观察对象的变量**，但**是其变量（引用）本身是不可观察的。**

#### 不正确的做法：在跟踪函数**外部使用**可观察属性
...
第一个改变将会被捕捉，`message.author` 和 `author` 是同一个对象，属性 `.name` 在 autorun 中被使用了，可以触发响应。 但是，第二个改变并 **不会** 被捕捉，因为 `message.author` 并没有被 `autorun` 跟踪，`autorun` 跟踪的是 `author.name`，autorun 依然会继续使用之前定义的“老的” `author`。
...
在上面的例子中，更新的信息的标题并不会被打印出来，因为他没有在 autorun 中被使用。 **autorun 只会依赖于** `message`，`message` **不是一个可观察对象，它只是一个变量**。换句话说，MobX 所关心的 `title` 并没有被 `autorun` 使用。
...
使其能够被**正常跟踪的方法**是确保**将不可变数据或者保护性副本传递**给 `console.log`。因此下面的所有方法都能使 `message.title` 的变化被响应：
...
这将会引发符合预期的响应。`.length` 也会被认为是一个属性。 注意：**发生在该数组中的 *****任何***** 变化都会引发响应**。 **数组不是按索引或属性（如可观察对象和 maps）跟踪的**，而是作为一个整体跟踪的。
...
这个实例将会对上面的示例数据作出响应，因为**数组索引算作属性访问**。但 **前提** 是提供的 `index < length`。 **MobX 不跟踪尚未存在的数组索引**。 因此，**请始终**使用 `.length` 检查（数组越界检查）来保护基于数组索引的访问。

#### 正确的做法：在跟踪**函数中访问数组的方法**
...
这将会引发符合预期的响应。所有**的数组索引赋值都会被检测到，** 但只有在`index <= length` 的情况下。
...
这 **会引发** 响应。可观察字典（map）**支持跟踪可能还不存在键值对**。 请注意，这将在最初打印出 `undefined`。 你可以通过使用 `twitterUrls.has("Sara")` 检查键值对在字典中是否存在。 所以在没有代理（Proxy）支持的环境下，**动态的键值对集合总是使用可观察字典来实现**。如果你有代理（Proxy）支持，也可以使用可观察字典， 当然你也可以选择使用普通对象。
...
这将会引发响应。尽管 `author.name` 并没有被接收了 `upperCaseAuthorName` 的 `autorun` 函数直接引用，MobX 依然会跟踪发生在 `upperCaseAuthorName` 中的引用，**因为这个引用发生在 autorun 的 *****执行过程***** 中。**
...
看看 [异步行为](https://www.mobxjs.com/actions#asynchronous-actions "异步行为") 章节获得更多理解。
...
当你在一个支持代理（Proxy）的环境中运行 React 时，这 **会引发** 响应。 请注意，**这仅适用于使用** `observable` 或 `observable.object` 创建的对象。**类实例对象上后续增加的新属性将不会自动成为可观察的**。
...
这将 **不会** 引发响应，**MobX 不会在跟踪开始时对不存在的可观察属性做出响应**。 如果**这两个语句被交换，或者任何其他可观察的语句导致 ****`autorun`**** 重新运行，****`autorun`**** 也将开始跟踪 ****`age`****。**
...
看看 [集合工具API](https://www.mobxjs.com/api#collection-utilities- "集合工具API") 获取更多细节。
