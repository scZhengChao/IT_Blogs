...
![](./assets/image/image_x4AJbEIfa5.webp)
...
<script>
    ...
    const dragView  = document.querySelector('.drag')
    // 图形被校验区域
    const check = document.querySelector('.check-child')
    const checkParent = document.querySelector('.check')
    const left = checkParent.getBoundingClientRect().left
    // 通过校验回调
    const success = () => {
        console.log('通过校验');
    }
    ...
    const dragMouseMove = event => {
        // 获取当前 x 轴坐标
        const { pageX,offsetX ,clientX,screenX}  = event
         const selfWith = 50/2
        let tranX = pageX - left -selfWith

        if(pageX < left + selfWith){
            tranX = 0
        }
        if(pageX > 400+left-selfWith){
            tranX = 400 - 50
        }
 
        // 修改可移动盒子的 x 轴坐标
        drag.style.transform = `translateX(${tranX}px)`

        // 修改被校验区域坐标
        check.style.transform = `translateX(${tranX}px)`
    ...
    const dragMouseUP = event => {
        ...
        if(pageX < 278 || pageX > 285){
            ...
            const animationEnd = ()=>{
                ...
                // 移出动画结束监听
                drag.removeEventListener("animationend", animationEnd)
            }
            // 添加动画结束监听
            drag.addEventListener("animationend", animationEnd)
        }
    }
