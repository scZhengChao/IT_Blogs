...
其实这是一项**很古老的技术，很早之前服务端就是通过 JSP、PHP 等模版引擎，渲染填充数据的模版，产生 html 返回的。只不过这时候没有组件的概念。**
...
![](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/13108680a6704cc58f7f21c3c1c5b460~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=882\&h=62\&s=25215\&e=png\&b=ffffff)

当然，这里应该有个 http 的 server，把组件 renderToString 的结果拼接成 html 返回。这里省略了。

假设下面就是服务端返回的 SSR 出的 html：

![](./image/image_CbaOcB4cvl.png)
...
![](./image/image_RjSAocI8vT.png)

组件的话就传入参数执行：

![](./image/image_U4_fG2kmRR.png)

元素的话就拼接字符串：

![](./image/image_KY6UPB2_LZ.png)

![](./image/image_zadgcBG9Ts.png)

这样递归渲染一遍，结果就是字符串了：

![](./image/image_M4qMBqP4dp.png)
...
![](./image/image_CRC3JRYkQp.png)

我们也经常把 React Element 叫做 vdom。

react 会把 vdom 转成 fiber 的结构，这个过程叫做 reconcile：

![](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/eb2f0d661068433da1f094336e4ed5e6~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1304\&h=740\&s=256608\&e=png\&b=fefdfd)
