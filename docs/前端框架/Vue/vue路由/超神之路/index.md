...
- [目录](#目录)
- [pushState/replaceState/popstate 解析](#pushStatereplaceStatepopstate-解析)
  - [pushState/replaceState() 方法](#pushStatereplaceState-方法)
- [vue-router 实现原理](#vue-router-实现原理)
  - [安装 vue-router](#安装-vue-router)
  - [router-view 实现视图更新](#router-view实现视图更新)
  - [创建 route 对象](#创建-route-对象)
- [route 跟 router 的区别](#route-跟-router-的区别)
- [通过路由元信息，设置登录](#通过路由元信息设置登录)
- [设置滚动行为](#设置滚动行为)
- [设置滚动行为](#设置滚动行为)
- [vue 路由 按需 keep-alive](#vue-路由-按需-keep-alive)
- [watch 监听路由变化](#watch-监听路由变化)
- [如何检测物理键返回](#如何检测物理键返回)
- [如何做出翻书效果](#如何做出翻书效果)
- [如何做一个优雅的路由分区](#如何做一个优雅的路由分区)

[ vue-router 的超神之路 本文是vue-router系列。这里从浏览器到vue-router原理到最佳实践都会有详细的讲解。由于篇幅较长，建议可以选择感兴趣的目录看。 https://mp.weixin.qq.com/s?\_\_biz=MzAxODE2MjM1MA==\&mid=2651557322\&idx=2\&sn=a2f3ae690b0c40006bc060e7cf01d6c2\&chksm=80255a0bb752d31dd374841bfb7b1becc7899692f9dc4abae0e478d232129635db195926d59c\&mpshare=1\&scene=1\&srcid=\&sharer\_sharetime=1572572815473\&sharer\_shareid=c581942ba12fd83f754283490bd7311e#](https://mp.weixin.qq.com/s?__biz=MzAxODE2MjM1MA==\&mid=2651557322\&idx=2\&sn=a2f3ae690b0c40006bc060e7cf01d6c2\&chksm=80255a0bb752d31dd374841bfb7b1becc7899692f9dc4abae0e478d232129635db195926d59c\&mpshare=1\&scene=1\&srcid=\&sharer_sharetime=1572572815473\&sharer_shareid=c581942ba12fd83f754283490bd7311e# " vue-router 的超神之路 本文是vue-router系列。这里从浏览器到vue-router原理到最佳实践都会有详细的讲解。由于篇幅较长，建议可以选择感兴趣的目录看。 https://mp.weixin.qq.com/s?__biz=MzAxODE2MjM1MA==\&mid=2651557322\&idx=2\&sn=a2f3ae690b0c40006bc060e7cf01d6c2\&chksm=80255a0bb752d31dd374841bfb7b1becc7899692f9dc4abae0e478d232129635db195926d59c\&mpshare=1\&scene=1\&srcid=\&sharer_sharetime=1572572815473\&sharer_shareid=c581942ba12fd83f754283490bd7311e#")
...
> “先了解了浏览器的history原理，才能更好的结合vue-router源码一步步了解它的实现。如果这块已经有了解可以直接跳过。
...
- URL — 该参数定义了新的历史URL记录。注**意，调用 pushState() 后浏览器并不会立即加载这个URL，但可能会在稍后某些情况下加载这个URL，** 比如在用户重新打开浏览器时。新URL不必须为绝对路径。如果新URL是相对路径，那么它将被作为相对于当前URL处理。新URL必须与当前URL同源，否则 pushState() 会抛出一个异常。该参数是可选的，缺省为当前URL。
...
然后，在 Vue.mixin中注入 beforeCreate钩子函数，每个组件都会调用 registerInstance， 通过 Vue.util.defineReactive

将 \_route进行监听，这样每次进入到新的页面就会设置当前的路由。
...
讲完原理给大家捋一下 route 跟 router 的区别，通过源码很容易看出他们的不同\$router是VueRouter的一个对象，通过Vue.use(VueRouter)和VueRouter构造函数得到一个router的实例对象，这个对象中是一个全局的对象，他包含了所有的路由包含了许多关键的对象和属性。

\$route

 就是一个路由的对象，我们通过 createRoute 创建出来的 route 对象，里面包括
...
比如你有这样列表页，点击进去是一个详情页，然后返回的时候列表刷新了，找不到原来的位置，这种时候对用户的体验非常不好。
...
随着项目的增大，项目中的页面可能达到好几十个，甚至更多，那么如何将这些页面进行管理呢？
