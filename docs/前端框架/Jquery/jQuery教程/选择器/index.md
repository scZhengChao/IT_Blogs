...
总之jQuery的**选择器不会返回**`undefined`或者`null`，这样的好处是**你不必在下一行判断**`if (div === undefined)`。
...
要注意的是，选出来的元素是**按照它们在HTML中出现的顺序排列**的，而且不会有重复元素。例如，`<p class="red green">`不会被上面的`$('p.red,p.green')`选择两次。

[层级选择器](./层级选择器/index.md "层级选择器")

[查找和过滤](./查找和过滤/index.md "查找和过滤")

[遍历 方法](<./遍历 方法/index.md> "遍历 方法")
