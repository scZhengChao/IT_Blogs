...
我们在IO的[Filter模式](https://liaoxuefeng.com/books/java/io/filter/index.html "Filter模式")一节中其实已经讲过装饰器模式了。在Java标准库中，`InputStream`是抽象类，`FileInputStream`、`ServletInputStream`、`Socket.getInputStream()`这些`InputStream`都是最终数据源。
...
![](./assets/image/image_oLV9m0AVbW.webp)
