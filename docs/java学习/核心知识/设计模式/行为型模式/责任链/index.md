...
![](./assets/image/image_yszQv9yuER.webp)
...
此外，责任链模式有很多变种。**有些责任链的实现方式是通过某**个`Handler`手动调用下一个`Handler`来传递`Request`，例如：
...
还有一些责任链模式，每个`Handler`都有机会处理`Request`，通**常这种责任链被称为拦截器（Interceptor）或者过滤器**（Filter），它的目的不是找到某个`Handler`处理掉`Request`，而是每个`Handler`都做一些工作，比如：
...
责任链模式的好**处是添加新的处理器或者重新排列处理器非常容易；**
