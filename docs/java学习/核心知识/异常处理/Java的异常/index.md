...
异常是一种`class`，**因此它本身带有类型信息。异常可以在任何地方抛出，但**只需要在上层捕获，这样就和方法调用分离了：
...
![](./assets/image/image_5UJLB7yOhV.webp)
...
可见，只要是方法声明的Checked Exception，不在调用层捕获，也必须在更高的调用层捕获。所有未捕获的异常，最终也必须在`main()`方法中捕获，不会出现漏写`try`的情况。这是**由编译器保证的。** \*\*`main()`****方法也是最后捕获****`Exception`\*\***的机会。**
