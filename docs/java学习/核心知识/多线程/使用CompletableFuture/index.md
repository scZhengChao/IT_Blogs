...
如果只是实现了**异步回调机制**，我们还看不出`CompletableFuture`相比`Future`的优势。`CompletableFuture`**更强大的功能是**，多个`CompletableFuture`**可以串行执行**，例如，定义两个`CompletableFuture`，第一个`CompletableFuture`根据证券名称查询证券代码，第二个`CompletableFuture`根据证券代码查询证券价格，这两个`CompletableFuture`实现串行操作如下：
...
![](./assets/image/image_6HCaGO7Fh3.webp)
