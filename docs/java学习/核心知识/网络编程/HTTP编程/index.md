...
![](./assets/image/image_vJ11H6v5B-.webp)
...
此外，`GET`请求**的参数必须附加在URL上，并以URLEncode方式编码**，例如：`http://www.example.com/?a=1&b=K%26R`，参数分别是`a=1`和`b=K&R`。因为URL的长度限制，`GET`请求的参数不能太多，而`POST`请求的参数就没有长度限制，因为`POST`请求的参数必须放到Body中。并且，`POST`请求的参数不一定是URL编码，可以按任意格式编码，只需要在`Content-Type`中正确设置即可。常见的发送JSON的`POST`请求如下：
...
**响应的第一行总**是`HTTP版本 响应代码 响应说明`，例如，`HTTP/1.1 200 OK`表示版本是`HTTP/1.1`，**响应代码**是`200`，**响应说明是**`OK`。客户端只依赖响应代码判断HTTP响应是否成功。HTTP有固定的响应代码：
...
![](./assets/image/image_l1W-01hOOr.webp)
...
![](./assets/image/image_HwOUTNhXFC.webp)
