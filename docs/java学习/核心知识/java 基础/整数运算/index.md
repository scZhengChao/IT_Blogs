...
要特别注意，整数由于存在范围限制，如果计算结果超出了范围，就会产生溢出，而溢出\_不会出错\_，却会得到一个奇怪的结果：
...
![](./assets/image/image_8qpP2KLyu3.webp)
...
![](./assets/image/image_kFUq14wMTF.webp)
...
也可以将结果强制转型，即将**大范围的整数转型为小范围的整数**。强制转型使用`(类型)`，例如，将`int`强制转型为`short`：
...
要注意，**超出范围的强制转型会得到错误的结果，**原因是转型时，`int`的两**个高位字节直接被扔掉，仅保留了低位的两个字节：**
