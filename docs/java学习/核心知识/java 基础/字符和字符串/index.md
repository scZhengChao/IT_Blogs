...
因为Java在内存中总是使用`Unicode`表示字符，所以，一个英文字符和一个中文字符都用一个`char`类型表示 \*\*，它们都占用两个字节。**要**显示一个字符的Unicode编码，只需将`char`****类型直接赋值给****`int`\*\***类型即可：**
...
如果用`+`连接字符串和其他数据类型，会将**其他数据类型先自动转型为字符串，再连接**：
...
![](./assets/image/image_M8Gtww2BTB.webp)

紧接着，执行`s = "world";`时，JVM虚拟机先创建字符串`"world"`，然后，把字符串变量`s`指向它：

![](./assets/image/image_oyCcjBU9Dz.webp)
