# Bean

## 目录

- [为什么要学](#为什么要学)
- [学什么  ](#学什么)
- [怎么学  ](#怎么学)
- [Spring简介  ](#Spring简介)
- [Spring Framework系统架构  ](#Spring-Framework系统架构)
- [核心概念  ](#核心概念)

# 为什么要学

![](./image/image_bnMAwEPnRy.png)

学什么

![](./image/image_S7HEyrFrzj.png)

怎么学

![](./image/image_XE1edsJ2AS.png)

Spring简介

![](./image/image_kk3hqAB5Zt.png)

![](./image/image_VqXFSIt2_Z.png)

Spring Framework系统架构

![](./image/image_smijQm9crX.png)

![](./image/image_KYkQCsCnvO.png)

![](./image/image_53I55KDHd9.png)

核心概念

![](./image/image_55ySiGeCkU.png)

![](./image/image_IkAxiwNBXA.png)

![](./image/image_nsLziWoJKT.png)

[Ioc 案例](<./Ioc 案例/index.md> "Ioc 案例")

[DI](./DI/index.md "DI")

[](./bean配置--/index.md)

[](./bean实例化-/index.md)

[](./依赖注入方式-/index.md)

[](./依赖自动装配--/index.md)

[](./集合注入--/index.md)

[](./Bean的生命周期-/index.md)

[第三方资源管理](./第三方资源管理/index.md "第三方资源管理")

[核心容器总结](./核心容器总结/index.md "核心容器总结")

[bean注解开发](./bean注解开发/index.md "bean注解开发")

[bean作用范围](./bean作用范围/index.md "bean作用范围")

[bean自动装配](./bean自动装配/index.md "bean自动装配")

[](./XML配置比对注解配置-/index.md)

[Spring整合MyBatis](./Spring整合MyBatis/index.md "Spring整合MyBatis")

[Spring整合jUnit](./Spring整合jUnit/index.md "Spring整合jUnit")
