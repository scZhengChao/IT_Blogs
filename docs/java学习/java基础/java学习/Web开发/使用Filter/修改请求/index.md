...
Filter**可以对请求进行预处理**，因此，我们可以把很多公共预处理逻辑放到Filter中完成。
...
但是要保证文件上传的完整性怎么办？在[哈希算法](https://liaoxuefeng.com/books/java/security/hash/index.html "哈希算法")一节中，我们知道，如果在上传文件的同时，把文件的哈希也传过来，服务器端做一个验证，就可以确保用户上传的文件一定是完整的。
...
这个时候，我们需要一个“伪造”的`HttpServletRequest`，具体做法是使用[代理模式](https://liaoxuefeng.com/books/java/design-patterns/structural/proxy/index.html "代理模式")，对`getInputStream()`和`getReader()`返回一个新的流：
