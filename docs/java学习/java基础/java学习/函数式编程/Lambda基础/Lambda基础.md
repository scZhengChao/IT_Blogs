...
在Java程序中，我们经常遇到**一大堆单方法接口，即一个接口只定义了一个方法：**
...
虽然`Comparator`接口有很多方法，但只有一个抽象方法`int compare(T o1, T o2)`，其他的方法都是`default`方法或`static`方法。另外注意到`boolean equals(Object obj)`是`Object`定义的方法，不算在接口方法内。因此，`Comp``arator`也是一个`FunctionalInterface`。
