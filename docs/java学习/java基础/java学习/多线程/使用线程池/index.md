...
线程池在程序结束的时候要关闭。使用`shutdown()`方法关闭线程池的时候，**它会等待正在执行的任务先完成，然后再关闭**。`shutdownNow()`会**立刻停止正在执行的任务**，`awaitTermination()`则会**等待指定的时间让线程池关闭**。
...
![](./image/image_H_pIgfsHNv.png)

而FixedDelay是指，**上一次任务执行完毕后，等待固定的时间间隔，再执行下一次任务：**

![](./image/image_ZXRyKtcm58.png)
