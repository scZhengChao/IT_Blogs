...
仔细思考，我们可以得出结论：`class`（包括`interface`）的**本质是数据类型**（`Type`）。**无继承关系的数据类型无法赋值：**
...
![](./image/image_tSyrZ6I13V.png)

**一个**\*\*`Class`****实例包含了该****`class`\*\***的所有完整信息：**

![](./image/image_V2L1x0Rbpi.png)
...
JVM在执行`Java`程序的时候，**并不是一次性把所有用到的class全部加载到内存，而是第一次需要用到class时才加载**。例如：
...
这就是为什么我们只需要把`Log4j`的`jar`包放到`classpath`中，`Commons Logging`就会自动使用Log4j的原因。
