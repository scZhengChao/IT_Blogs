...
在了解`Maven`之前，我们先来看看一个`Java`项目需要的东西。首先，**我们需要确定引入哪些依赖包**。例如，如果我们需要用到[commons logging](https://commons.apache.org/proper/commons-logging/ "commons logging")，我们就必须把commons logging的jar包放入`classpath`。如果我们还需要[log4j](https://logging.apache.org/log4j/ "log4j")，就需要把log4j相关的jar包都放到`classpath`中。这些就是依赖包的管理。
...
![](./assets/image/image_J7X8Kr_L0j.webp)

项目的根目录`a-maven-project`是项目名，它有一个项目描述文件`pom.xml`，存放Java源码的目录是`src/main/java`，存放资源文件的目录是`src/main/resources`，存放测试源码的目录是`src/test/java`，存放测试资源的目录是`src/test/resources`，最后，所有编译、打包生成的文件都放在`target`目录里。这些就是一个Maven项目的标准目录结构。

所有的**目录结构都是约定好的标准结构，我们千万不要随意修改目录结构。使用标准结构不需要做任何配置，Maven就可以正常使用。**
...
要安装Maven，可以从[Maven官网](https://maven.apache.org/ "Maven官网")下载最新的Maven 3.9.x，然后在本地解压，设置几个环境变量：
...
![](./assets/image/image_d_D10PE7kw.webp)
