...
使用`Spring`的`IoC`容器，实际上就是**通过类似XML这样的配置文件**，把我们**自己的Bean的依赖关系描述出来，然后让容器来创建并装配Bean。一旦容器初始化完毕，我们就直接从容器中获取Bean使用它们。**
...
此外，`AppConfig`还标注了`@ComponentScan`，它\*\*告诉容器，****自动搜索当前类所在的包以及子包****，把所有标注为`@Component`****的Bean自动创建出来，并根据****`@Autowired`\*\***进行装配。**

整个工程结构如下：

![](./assets/image/image_KdeFgrXGqu.webp)
