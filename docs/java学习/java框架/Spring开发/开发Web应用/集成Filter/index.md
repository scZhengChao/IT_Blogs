...
但是，在Servlet规范中，我们还可以[使用Filter](https://liaoxuefeng.com/books/java/web/filter/index.html "使用Filter")。如果要在Spring MVC中使用`Filter`，应该怎么做？
...
现在问题来了：在Spring中\*\*创建的这个`AuthFilter`\*\***是一个普通Bean，Servlet容器并不知道，所以它不会起作用。**

如果我们直接在`web.xml`中声明这个`AuthFilter`，注意到`AuthFilter`的实例**将由Servlet容器而不是Spring容器初始化**，**因此，** \*\*`@Autowire`根本不生效，\*\*用于登录的`UserService`成员变量永远是`null`。
...
当`DelegatingFilterProxy`生效后，它**会自动查找注册在`ServletContext`****上的Spring容器，再试图从容器中查找名为****`authFilter`****的Bean，也就是我们用****`@Component`****声明的****`AuthFilter`。**
...
![](./assets/image/image_7veQ0hf0mO.webp)
...
当**一个Filter作为Spring容器管理的Bean存在时，** 可以通过`DelegatingFilterProxy`间接地引用它并使其生效。
