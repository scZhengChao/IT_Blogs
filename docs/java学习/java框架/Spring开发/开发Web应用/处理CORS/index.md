...
- [使用@CrossOrigin](#使用CrossOrigin)
- [使用CorsRegistry](#使用CorsRegistry)
- [使用CorsFilter](#使用CorsFilter)
- [测试](#测试)
- [小结](#小结)
...
关于CORS的详细信息可以参考[MDN文档](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Access_control_CORS "MDN文档")，这里不再详述。
...
这种方式可以创建一个全局CORS配置，如果仔细地设计URL结构，那么可以**一目了然地看到各个URL的CORS规则，推荐使用这种方式配置CORS。**

### 使用CorsFilter

第三种方法是使用Spring提供的`CorsFilter`，我们在[集成Filter](https://liaoxuefeng.com/books/java/spring/web/filter/index.html "集成Filter")中详细介绍了将Spring容器内置的Bean暴露为Servlet容器的Filter的方法，由于这种配置方式需要修改`web.xml`，也比较繁琐，所以推荐使用第二种方式。
...
![](https://liaoxuefeng.com/books/java/spring/web/cors/cors-error.png)
