...
- [AMQP协议](#AMQP协议)
- [使用RabbitMQ](#使用RabbitMQ)
- [小结](#小结)

前面我们讲了ActiveMQ Artemis **，它实现了JMS的消息服务协议。JMS是JavaEE的消息服务标准接口，** 但是，**如果Java程序要和另一种语言编写的程序通过消息服务器进行通信，那么JMS就不太适合了。**

**AMQP是一种使用广泛的独立于语言的消息协议，**它的全称是Advanced Message Queuing Protocol，即**高级消息队列协议，它定义了一种二进制格式的消息流，任何编程语言都可以实现该协议。**实际上，Artemis也支持AMQP，但实际应用最广泛的AMQP**服务器是使用**[**Erlang**](https://www.erlang.org/ "Erlang")**编写的**[**RabbitMQ**](https://www.rabbitmq.com/ "RabbitMQ")**。**

### 安装RabbitMQ

我们先从RabbitMQ的官网[下载](https://www.rabbitmq.com/download.html "下载")并安装RabbitMQ，安装和启动RabbitMQ请参考官方文档。要验证启动是否成功，可以访问RabbitMQ的管理后台[http://localhost:15672](http://localhost:15672/ "http://localhost:15672")，如能看到登录界面表示RabbitMQ启动成功：

![](https://liaoxuefeng.com/books/java/springboot/integration/rabbitmq/rabbitmq.jpg)
...
![](./assets/image/image_TA3qWQgyci.webp)
...
![](./assets/image/image_EIokTi-v0c.webp)
...
![](https://liaoxuefeng.com/books/java/springboot/integration/rabbitmq/exchange.png)
...
![](https://liaoxuefeng.com/books/java/springboot/integration/rabbitmq/exchange2.png)

上述Binding的规则稍微复杂一点，当发送消息给`login`这个Exchange时，如果消息没有指定Routing Key，则被投递到`q_app`和`q_mail`，如果消息指定了Routing Key="login\_failed"，那么消息被投递到`q_sms`。
...
`MessageConverter`**用于将Java对象转换为RabbitMQ的消息。默认情况下，Spring Boot使用**`SimpleMessageConverter`，只能发送`String`和`byte[]`类型的消息，不太方便。使用`Jackson2JsonMessageConverter`，我们就**可以发送JavaBean对象，由Spring Boot自动序列化为JSON并以文本消息传递。**
...
RabbitMQ还提供了**使用Topic的Exchange**（此Topic指消息的标签，并非JMS的Topic概念），**可以使用**\*\*`*`\*\***进行匹配并路由。可见，掌握RabbitMQ的核心是理解其消息的路由规则。**
...
Spring Boot提供了`AMQP`的集成，默认使用`RabbitMQ`作为`AMQP`消息服务器。
