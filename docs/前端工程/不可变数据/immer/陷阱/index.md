...
- [不要重新分配 recipe 参数](#不要重新分配-recipe-参数)
- [Immer 只支持单向树](#Immer-只支持单向树)
- [永远不要从 producer 那里显式返回undefined](#永远不要从-producer-那里显式返回undefined)
- [不要修改特殊对象](#不要修改特殊对象)
- [类应该是可 draft 的或不可变的](#类应该是可-draft-的或不可变的)
- [只有有效的索引和长度可以在数组上改变](#只有有效的索引和长度可以在数组上改变)
- [只有来自 state 的数据会被 draft](#只有来自-state-的数据会被-draft)
- [Immer patches 不一定是最优的](#Immer-patches-不一定是最优的)
- [始终使用嵌套 producers 的结果](#始终使用嵌套-producers-的结果)
- [Drafts 在引用上不相等](#Drafts-在引用上不相等)
...
永远不要重新分配`draft`参数（例如：`draft = myCoolNewState`）。相反 **，要么修改 draft，要么返回新状态**。请参阅[从 producers 返回数据](https://immerjs.github.io/immer/zh-CN/return "从 producers 返回数据")。
...
Immer[不支持特殊对象](https://github.com/immerjs/immer/issues/504 "不支持特殊对象")比如 window\.location.

### 类应该是可 draft 的或不可变的

您将需要使自己的类能与 Immer 一起正常工作。有关该主题的文档，请查看有关使用[复杂对象](https://immerjs.github.io/immer/zh-CN/complex-objects "复杂对象")的部分。
...
Immer 中的 draft 对象包装在`Proxy`中，因此您不能使用`==`或`===`来测试**原始对象与其 draft 之间的相等性**（例如，当匹配数组中的特定元素时）。相反，您可以使用`original`助手：
