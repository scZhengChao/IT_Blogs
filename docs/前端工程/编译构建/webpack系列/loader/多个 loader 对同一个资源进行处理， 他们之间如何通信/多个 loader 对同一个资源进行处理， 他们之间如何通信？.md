# 多个 loader 对同一个资源进行处理， 他们之间如何通信？

## 目录

- [使用 this.data](#使用-thisdata)
- [使用自定义属性](#使用自定义属性)
- [注意事项](#注意事项)

**关键词**：`webpack loader` 通信

在 `webpack` 中，`loader` 之间传递数据的常见方式是**通过资源文件（即要处理的源文件本身）的内容**。*每个 **`loader`** 接收上一个 **`loader`** 的处理结果作为输入，**并提供自己的输出给下一个** **`loader`**。这种方式适用于大多数使用场景*。然而，在某些情况下，loader 需要在它们之间共享额外的状态或数据，而不仅仅是文件内容。对于这种需求，`webpack` 提供了一种机制，允许 `loader` 之间共享数据。
