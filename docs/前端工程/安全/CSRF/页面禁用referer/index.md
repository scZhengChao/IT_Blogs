...
- [分析](#分析)
  - [危害性](#危害性)
- [解决方式](#解决方式)
  - [直接不加载第三方统计](#直接不加载第三方统计)
  - [禁止 referer 头部](#禁止-referer-头部)
- [论页面禁止 referer 的 6 种方式](#论页面禁止-referer-的-6-种方式)
  - [1. head 标签中添加 meta属性](#1-head-标签中添加-meta属性)
    - [例子](#例子)
    - [兼容性](#兼容性)
  - [2. 添加ReferrerPolicy属性](#2-添加ReferrerPolicy属性)
    - [兼容性](#兼容性)
  - [3. 通过 rel=’noreferrer‘](#3-通过-relnoreferrer)
  - [4. 代理模式](#4-代理模式)
  - [5. 外链 通过 iframe 来打开](#5-外链-通过-iframe-来打开)
  - [6. 外链通过新窗口打开](#6-外链通过新窗口打开)
  - [推荐一个第三方库 noreferrer.js](#推荐一个第三方库-noreferrerjs)
- [最后的解决方案](#最后的解决方案)

```javascript 

1. a标签增加属性： <a rel="noreferrer" href="....">下载</a>
 
2. head中增加：  <meta name="referrer" content="never">
```
...
![](./assets/image/image_rPT2qTFamZ.webp)
...
最好的方式就是这个**页面在加载外部资源的时候，禁止 ****`referer`**** 头部**。 后面我们也是用这种方式来处理的。
...
![](./assets/image/image_VSpIHJ10DV.webp)
...
![](./assets/image/image_aQi_U-myKO.webp)

然后对应请求的 `Referrer Policy` 变成 `origin`:

![](https://kebingzao.com/2020/05/07/referrer-leakage/4.png)
...
![](./assets/image/image_Lr6yyDp599.webp)

### 2. 添加ReferrerPolicy属性

添加 `meta` 标签相当于对文档中的所有链接都取消了 `referer` ，而 `ReferrerPolicy` 则**更精确的指定了某一个资源**的 `referer` 策略。关于这个策略的定义可以参照 MDN。比如我想只对某一个图片取消referrer，如下编写即可:
...
|   |                                                       |
| - | ----------------------------------------------------- |
| 1 | \<a href="xxxx.html"  referrerPolicy="no-referrer" /> |
...
![](./assets/image/image_aq5gZvX03R.webp)
...
|   |                                           |
| - | ----------------------------------------- |
| 1 | \<a href="xxxx.html"  rel="noreferrer" /> |

当然目前兼容的有限, 只支持 `<a>`, `<area>`, `<form>` 这三个元素:

> `, `\<area>`, and `\<form>\` elements instructs the browser, when navigating to the target resource, to omit the Referer header and otherwise leak no referrer information — and additionally to behave as if the noopener keyword were also specified.
>
> **mozilla**[developer.mozilla.org/en-US/docs/Web/HTML/Link\_types/noreferrer](https://developer.mozilla.org/en-US/docs/Web/HTML/Link_types/noreferrer "developer.mozilla.org/en-US/docs/Web/HTML/Link_types/noreferrer")

通过 [can i use noreferrer](https://caniuse.com/#search=noreferrer "can i use noreferrer") 可以看到支持的浏览器版本:

![](./assets/image/image_lnQWLF7yUO.webp)
...
提供跨浏览器支持的更好的办法是使用一个第三方的库 [noreferrer.js](https://github.com/knu/noreferrer/blob/master/noreferrer.js "noreferrer.js")，它可以自动识别浏览器并选择最优方案。

不过这边需要注意一点的是, 对于某些几乎不支持的特性浏览器，比如 `Opera`，`noreferrer.js` 的解决方案是利用 `google` 的`url`中转。在国内的网络环境下，你懂的。。。 所以可以自己搭建一个跳转的页面，或者用其他站点的url跳转接口。
...
![](https://kebingzao.com/2020/05/07/referrer-leakage/8.png)
