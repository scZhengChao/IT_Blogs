...
- [静态资源组织](#静态资源组织)
  ...
  - [利用缓存](#利用缓存)
    ...
    - [强缓存](#强缓存)
  - [缓存更新问题](#缓存更新问题)
  - [覆盖式发布引发的问题](#覆盖式发布引发的问题)
  - [与 CDN 结合](#与-CDN-结合)
  - [静态资源组织总结](#静态资源组织总结)
- [自动化构建](#自动化构建)
- [前端发布服务 - 预发环境、版本管理（秒级回滚）、小流量、灰度、AB测试](#前端发布服务---预发环境版本管理秒级回滚小流量灰度AB测试)
  - [静态资源的加工](#静态资源的加工)
  - [Pre 环境、灰度上线的常见实现](#Pre-环境灰度上线的常见实现)
    - [方案一 Nginx 层动态转发](#方案一-Nginx-层动态转发)
    - [方案二 动态配置 + 服务端转发](#方案二-动态配置--服务端转发)
  - [前端发布服务实现与设计](#前端发布服务实现与设计)
- [关于部署的总结](#关于部署的总结)
  - [静态资源组织部分](#静态资源组织部分)
  - [自动化部署部分](#自动化部署部分)
  - [前端部署 & 静态资源加工](#前端部署--静态资源加工)
  - [前端发布服务](#前端发布服务)
- [其他](#其他)
  - [参考资料](#参考资料)
...
![](./assets/image/image_eWOWyeV_Xh.webp)

此时，使用协商缓存后，`Network` 大致变成了这样：

![](./assets/image/image_BFQLPFnu6b.webp)
...
![](./assets/image/image_MpyV3JlVcS.webp)

> 注意，缓存生效期间，浏览器是【自言自语】，和服务器无关。

此时，设置强缓存后，Network 大致变成了这样：

![](./assets/image/image_c57z1ojnZr.webp)
...
![](./assets/image/image_1slfJEkrMZ.webp)

相信大家很快就能得出一种思路，给资源加版本号！比如通过 `query` 加版本号，每次上线统一改版本号就搞定了。此时 `HTML` 变成如图：

![](./assets/image/image_P4kCdJ31Bg.webp)
...
![](./assets/image/image__KKJ4-xE4x.webp)
...
![](./assets/image/image_ms1Ma3hSc9.webp)
...
![](./assets/image/image_2qV5OFm_Ll.webp)
...
![](./assets/image/image_k2SfUV6Jw8.webp)
...
![](./assets/image/image_U0YN4lNmcB.webp)
...
>
> 备注 3：`publicPath` 详解[**webpack.docschina.org/configurati…**](http://webpack.docschina.org/configurati… "webpack.docschina.org/configurati…")\[3]
>
...
案二是我们将热锅串串和冷锅串串分开打包，让热锅不犯冷锅。再设计一些机制，比如携带了香蕉糖果（cookie）的同学给跷脚牛肉锅，讲港东话的同学福建美食锅，四川地域的同学随机给火锅干锅汤锅鱼火锅。岂不乐哉？
...
![](./assets/image/image_6FqqbTYjiB.webp)
...
![](./assets/image/image_OEucUz71Y4.webp)
...
![](./assets/image/image_Kz0CNKxsIc.webp)
...
![](./assets/image/image_jMJwTkErNB.webp)
...
Q：配套的，前端静态资源应该如何组织？A：搭配 Webpack 的Webpack\_HTML-Plugin & 配置 output publicPath等。
...
Q：如何实现一套前端代码，发布成多套环境产物？A：使用环境变量，将当前环境、CDN、CDN\_HOST、Version等注入环境变量中，构建时消费 & 将产物上传不同的CDN即可。

# 其他

如果想深入学习前端部署，下面是一些学习建议。

1. 学习负载均衡（要求：了解）。学习和了解负载均衡的原理、都有哪些配置玩法。如参考**大型网站架构系列：负载均衡详解**\[14]
2. 深入学习 HTTP（要求：熟练掌握） 如掌握常见的状态码、常见的 Header 及其深度应用、强缓存/协商缓存、HTTP2 的新增功能等等。尤其HTTP 1.1 和 HTTP 2.0。推荐书籍：**图解HTTP**\[15]**HTTP 权威指南**\[16]
...
5. 深度学习浏览器原理 （要求：精通） 一些资料：**从浏览器多进程到JS单线程，JS运行机制最全面的一次梳理**\[19]

### 参考资料

\[1]强缓存/协商缓存详解: [*https://juejin.cn/post/6844903838768431118*](https://juejin.cn/post/6844903838768431118 "https://juejin.cn/post/6844903838768431118")

\[2]消息摘要算法:[*https://www.baike.com/wikiid/1412805162344902915?search\_id=3k1jee3i202000\&prd=search\_sug\&view\_id=2p9w1v50ati000*](https://www.baike.com/wikiid/1412805162344902915?search_id=3k1jee3i202000\&prd=search_sug\&view_id=2p9w1v50ati000 "https://www.baike.com/wikiid/1412805162344902915?search_id=3k1jee3i202000\&prd=search_sug\&view_id=2p9w1v50ati000")

\[3][webpack.docschina.org/configurati…:](http://webpack.docschina.org/configurati…: "webpack.docschina.org/configurati…:")[*https://webpack.docschina.org/configuration/output/#outputpublicpath*](https://webpack.docschina.org/configuration/output/#outputpublicpath "https://webpack.docschina.org/configuration/output/#outputpublicpath")

\[4]详解webpack中的hash、chunkhash、contenthash区别:[*https://www.cnblogs.com/ajaemp/p/12915452.html*](https://www.cnblogs.com/ajaemp/p/12915452.html "https://www.cnblogs.com/ajaemp/p/12915452.html")

\[5]webpack 官方文档-缓存:[*https://webpack.docschina.org/guides/caching/*](https://webpack.docschina.org/guides/caching/ "https://webpack.docschina.org/guides/caching/")

\[6]漫话：如何给女朋友解释什么是反向代理？: [*https://juejin.cn/post/6844903782556368910*](https://juejin.cn/post/6844903782556368910 "https://juejin.cn/post/6844903782556368910")

\[7]nginx 之 proxy\_pass详解:[*https://www.jianshu.com/p/b010c9302cd0*](https://www.jianshu.com/p/b010c9302cd0 "https://www.jianshu.com/p/b010c9302cd0")

\[8]Docker:[*http://www.ruanyifeng.com/blog/2018/02/docker-tutorial.html*](http://www.ruanyifeng.com/blog/2018/02/docker-tutorial.html "http://www.ruanyifeng.com/blog/2018/02/docker-tutorial.html")

\[9]Jenkins:[*https://cloud.tencent.com/developer/article/1504850*](https://cloud.tencent.com/developer/article/1504850 "https://cloud.tencent.com/developer/article/1504850")

\[10]Gitlab webhook:[*https://segmentfault.com/a/1190000015437514*](https://segmentfault.com/a/1190000015437514 "https://segmentfault.com/a/1190000015437514")

\[11]前端项目自动化部署——超详细教程（Jenkins、Github Actions）:[*https://blog.csdn.net/weixin\_49592546/article/details/109352249*](https://blog.csdn.net/weixin_49592546/article/details/109352249 "https://blog.csdn.net/weixin_49592546/article/details/109352249")

\[12]iDeploy-为前端团队构建部署工程化而开发的一个持续交付平台:[*https://github.com/wdfe/ideploy*](https://github.com/wdfe/ideploy "https://github.com/wdfe/ideploy")

\[13]nginx配置rewrite指令详解:[*https://leokongwq.github.io/2016/11/23/nginx-rewrite.html*](https://leokongwq.github.io/2016/11/23/nginx-rewrite.html "https://leokongwq.github.io/2016/11/23/nginx-rewrite.html")

\[14]大型网站架构系列：负载均衡详解: [*https://www.jianshu.com/p/215b5575107c*](https://www.jianshu.com/p/215b5575107c "https://www.jianshu.com/p/215b5575107c")

\[15]图解HTTP: [*https://book.douban.com/subject/25863515/*](https://book.douban.com/subject/25863515/ "https://book.douban.com/subject/25863515/")

\[16]HTTP 权威指南: [*https://book.douban.com/subject/10746113/*](https://book.douban.com/subject/10746113/ "https://book.douban.com/subject/10746113/")

\[17]前端工程化：体系设计与实践:[*https://book.douban.com/subject/27605366/*](https://book.douban.com/subject/27605366/ "https://book.douban.com/subject/27605366/")

\[18]Webpack 官方文档: [*https://webpack.docschina.org/guides/*](https://webpack.docschina.org/guides/ "https://webpack.docschina.org/guides/")

\[19]从浏览器多进程到JS单线程，JS运行机制最全面的一次梳理: [*https://segmentfault.com/a/1190000012925872*](https://segmentfault.com/a/1190000012925872 "https://segmentfault.com/a/1190000012925872")
