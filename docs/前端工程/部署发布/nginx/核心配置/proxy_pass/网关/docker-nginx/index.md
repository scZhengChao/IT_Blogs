...
![](./image/image_HxxwnliMYA.png)

在 nest 服务的 handler 里注入 headers，打印一下：

![](./image/image_4hsyeUop-q.png)

然后浏览器访问下。

直接访问 nest 服务的话，是没有这个 header 的：

![](./image/image_LQ5is4TMsN.png)

![](./image/image_wJIUqPrFZo.png)
...
![](./image/image_K6EP1-catV.png)

把 nest 服务停掉，然后重新 npm run start

![](./image/image_G4WPJH0KAB.png)

3001 和 3002 端口各跑一个：

![](./image/image_d9yRutorby.png)
...
![](./image/image_EMeFlOYGXS.png)

这就是带权重的轮询。

我们再试下 ip\_hash 的方式；

![](./image/image_nHMlxWWISS.png)
...
![](./image/image_HuVA-ILUKL.png)

按 command + k，把 nest 服务的控制台日志清空下：

![](./image/image_KhIUyNrmhO.png)
