...
![](image_OUvca-l7Gw.png)

里面的 /usr/share/nginx/html/ 目录下面就是所有的静态文件。

双击点开 index.html 看看：

![](image_tZO2bnZygg.png)

![](image_gn3HffkXRY.png)
...
![](image_JOoIoXnLcI.png)

docker cp 这个命令就是用于在宿主机和容器之间复制文件和目录的。

![](image_-JkOEwD8UA.png)
...
![](image_Ab9AUQmNj8.png)

**但当目标目录存在的时候，docker 会把他复制到目标目录下面：**

![](image_FP-ND0HlyI.png)

我们需要先删除容器的这个目录，再复制：

![](image_mc49aJGGbt.png)
...
![](image_Zori-ReGfB.png)

location 和路径**之间加了个 =，代表精准匹配，** 也就是只有完全相同的 url 才会匹配这个路由。

![](image__kuZ9tcB5O.png)

![](image_Zyb8eP4qZA.png)

**不带 = 代表根据前缀匹配，后面可以是任意路径。**

![](image_5GkC6u2l32.png)

**这里的 \$uri 是取当前路径。**

![](image_mjOI3QYbbA.png)

![](image_hpqs-oPxwe.png)

然后**如果想支持正则，就可以加个 \~。**

![](image_NwiJ3LVsLX.png)

这里的正则语法不难看懂，就是 /aaaa/bbb 开头，然后中间是任意字符，最后 .html 结尾的 url。

![](image_oDSvf-msh6.png)

![](image_MHV6LJyqze.png)

**但是它是区分大小写的，比如这样就不行了：**

![](image_F10DtPwk98.png)
...
![](image_yVkJ9Z_USN.png)

这时候就有两个 /444 的路由了：

![](image_zYStgDPWox.png)
