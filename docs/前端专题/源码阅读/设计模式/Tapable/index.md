...
- [二、Tapable是什么？](#二Tapable是什么)
- [三、按照同步/异步分类](#三按照同步异步分类)
- [四、按照执行机制分类](#四按照执行机制分类)
- [五、基本使用](#五基本使用)
  - [5.1、SyncHook](#51SyncHook)
  - [5.2、SyncBailHook](#52SyncBailHook)
  - [5.3、SyncWaterfallHook](#53SyncWaterfallHook)
  - [5.4、SyncLoopHook](#54SyncLoopHook)
  - [5.5、AsyncParallelHook](#55AsyncParallelHook)
  - [5.6、AsyncParallelBailHook](#56AsyncParallelBailHook)
  - [5.7、AsyncSeriesHook](#57AsyncSeriesHook)
  - [5.8、AsyncSeriesBailHook](#58AsyncSeriesBailHook)
  - [5.9、AsyncSeriesWaterfallHook](#59AsyncSeriesWaterfallHook)
- [六、具体实现](#六具体实现)
  - [6.1、工欲善其事，必先利其器](#61工欲善其事必先利其器)
  - [6.2、核心思想](#62核心思想)
  - [6.3、如何生成运行函数](#63如何生成运行函数)
  - [6.4、手撕代码](#64手撕代码)
  - [6.5、为什么这么设计？](#65为什么这么设计)
- [七、总结](#七总结)

[tapable ](../../../../前端工程/工具库/tapable-/index.md "tapable ")

所有人都知道 [Webpack](https://link.juejin.cn?target=https://webpack.js.org/ "Webpack") 很复杂，但 `Webpack` 的源码却很优雅，是一个典型的**可插拔架构**，不仅逻辑清晰，而且灵活好扩展。近几年出来的一些构建工具，大多也都参考了 Webpack 的这种架构方式。

而实现这一切的核心就是借助了 [Tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "Tapable")。

关于 [Tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "Tapable") 的源码其实并没有多少代码量，学习它的原理首当其冲的一定是可以让你在日常 [Webpack Plugin](https://link.juejin.cn?target=https://webpack.js.org/concepts/plugins/ "Webpack Plugin") 开发中更得心应手，解决相关问题更加顺畅。
...
在本文中我们将会抛开 `Webpack`，在第 1 ～ 5 节主要是讲解基本原理和使用方式（奈何官方文档实在太简陋...），第 6 节则会以图文的形式深度分析 [Tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "Tapable") 的实现原理，熟悉使用的同学可跳过前面几节。

通篇将会采用**结论先行、自顶向下**的方式进行讲解，`注重实现思路，注重设计思想`，与 Webpack 完全解耦，可放心食用。

文中所涉及到的代码均放到个人 github 仓库中：[github.com/noBaldAaa/m…](https://link.juejin.cn?target=https://github.com/noBaldAaa/my-tapable "github.com/noBaldAaa/m…")

![](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/47117e5271e04dfc8cb0cab3ca10ca13~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=270\&h=270\&s=20037\&e=gif\&b=fcfcfc)

## 二、Tapable是什么？

[Tapable](https://link.juejin.cn?target=https://www.npmjs.com/package/tapable "Tapable")是一个类似于 Node.js 中的 [EventEmitter](https://link.juejin.cn?target=https://www.npmjs.com/package/events "EventEmitter") 的库，但它**更专注于自定义事件的触发和处理**。通过 `Tapable` 我们可以注册自定义事件，然后在适当的时机去执行自定义事件。

![](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/be77329e409a45d5bd184965ce8f5760~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=198\&h=187\&s=5899\&e=jpg\&b=f4f4f4)

举个例子🌰：类比到 `Vue` 和 `React` 框架中的生命周期函数，它们就是到了固定的时间节点就执行对应的生命周期，`tapable` 做的事情就和这个差不多，我们可以通过它**先注册一系列的生命周期函数**，然后在合适的时间点执行。
...
根据官方介绍，[tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "tapable") 使用起来还是挺简单的，只需三步：

1. \*\*实例化钩子函数（ **[**tapable**](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "tapable")**会暴露出各种各样的 Hook，这里先以同步钩子****`Synchook`\*\***为例）**
2. **注册事件**
3. **触发事件**
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0877aad167f241b49348be60056f7730~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=227\&h=191\&s=7382\&e=jpg\&b=f8f8f8)

从上面的例子中可以看出 [tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "tapable") 采用的是**发布订阅模式**，`通过 tap 函数注册监听函数，然后通过 call 函数按顺序执行之前注册的函数`。
...
另外，[tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "tapable") 中不仅有 `Synchook`，还有其他 八个 `Hook` :
...
![](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ce82751ba4d147a7aa406129ebe57026~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=192\&h=149\&s=17309\&e=png\&b=fbfbfb)
...
![](./image/image_4DRgpCqwL6.png)

对**同步钩子**来说， `tap` 方法是**唯一注册事件**的方法，通过 `call` 方法触发同步钩子的执行。

对**异步钩子**来说，可以通过 `tap`、`tapAsync`、`tapPromise`三种方式来注册，通过对应的 `callAsync`、`promise` 这两种方式来触发注册的函数。
...
`AsyncParallelHook`是一个**异步并行、基本类型**的 `Hook`，它与同步 `Hook` 不同的地方在于：

- 它会同时开启多个异步任务，而且需要通过 `tapAsync` 方法来注册事件（同步 `Hook` 是通过 `tap` 方法）
- 在执行注册事件时需要使用 `callAsync` 方法来触发（同步 `Hook` 使用的是 `call` 方法）

同时，在每个**注册函数的回调中，会多一个** `callback` 参数，它是一个函数。执行 `callback` 函数相当于告诉 `Hook` 它这一个异步任务执行完成了。
...
`AsyncParallelBailHook` 是一个**异步并行、保险类型**的 `Hook`，只要**其中一个有返回值，就会执行** `callAsync` 中的回调函数。
...
`AsyncSeriesHook` 是一个**异步、串行类型**的 `Hook`，只有前面的执行完成了，后面的才会一个接一个的执行。
...
![](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/80db468f99e14f2cb03057c3fd66ddb9~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=2580\&h=798\&s=294613\&e=png\&b=242424)

第三步：点击 Run and Debug，选择 Node.js 环境

![](./image/image_yFMV634wl_.png)

> （2）在 IDE 中快速的执行代码文件

第一步：在 IDE 的应用商店中下载插件 [Code Runner](https://link.juejin.cn/?target=https://marketplace.visualstudio.com/items?itemName=formulahendry.code-runner "Code Runner")：

![](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bd800382790b4d0ea04e39128e5af8d3~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=2512\&h=518\&s=121030\&e=png\&b=1e1e1e)

第二步：选择想要运行的文件，点击右键，选择 Run Code 选项：

![](./image/image_VV042xbLye.png)

第三步：在控制台中查看结果：

![](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4df7e67a1ce8438994b04c611b9940f4~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1830\&h=394\&s=84201\&e=png\&b=1f1f1f)

### 6.2、核心思想

这里以 `SyncHook` 的实现原理为例，其他的 `Hook` 也会整理一下思路，大家举一反三，重点在于理解思想。

![](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bb6fc1ee1e794a0e8760793457594f79~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=215\&h=215\&s=143976\&e=gif\&f=7\&b=bac5cc)

我们再回过头来看看 `SyncHook` 的用法，也就是这三步：

![](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/867ad31ffdac43329f2b81c6faee0441~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1278\&h=784\&s=141716\&e=png\&b=fff2ec)
...
这里官方的源码中是通过 [new Function()](https://link.juejin.cn/?target=https://zh.javascript.info/new-function "new Function()") 进行创建的，先了解一下 `new Function` 的语法：
...
这么做一部分原因是为**了极佳的性能考虑**，比如只有在执行 `call` 方法时才会**去动态生成执行函数**，如果**不执行则不处理**（**懒编译或者叫动态编译**）。

还有一部分原因**则是为了更加灵活**。别忘了，该库里面还有其他类型的 `Hook`，如果我们想要实现其他 `Hook`，只需要定义好各自的 `compiler` 函数就可以了。

另外，Webpack作者也提到过为什么采用 `new Function` 的方案，**一切都是为了性能考虑**，链接在这里：[github.com/webpack/tap…](https://link.juejin.cn/?target=https://github.com/webpack/tapable/issues/162 "github.com/webpack/tap…") ，有兴趣的可以去看看。

## 七、总结

本文从一个基础案例出发，先依次讲解了 [Tapable](https://link.juejin.cn/?target=https://www.npmjs.com/package/tapable "Tapable") 中各种类型 Hook 的基本用法和运行机制，接着再次回到最初的案例中，花了大量篇幅讲解 `Tapable` 的核心思想和实现思路。在这过程中不仅讲清楚了怎么去实现，更重要的是授人以渔，分析了为什么这么做。
