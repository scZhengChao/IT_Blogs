...
- [WebRTC 是什么？ ](#WebRTC-是什么-)
- [为什么我应该学习 WebRTC？ #](#为什么我应该学习-WebRTC-)
- [WebRTC 协议是一组其他技术的集合体 #](#WebRTC-协议是一组其他技术的集合体-)
  - [信令：peer 如何在 WebRTC 中找到彼此 #](#信令peer-如何在-WebRTC-中找到彼此-)
  - [使用 STUN/TURN 进行连接和 NAT 穿透 #](#使用-STUNTURN-进行连接和-NAT-穿透-)
  - [使用 DTLS 和 SRTP 加密传输层](#使用-DTLS-和-SRTP-加密传输层)
  - [通过 RTP 和 SCTP 进行点对点通信 #](#通过-RTP-和-SCTP-进行点对点通信-)
- [WebRTC 是一系列协议的集合](#WebRTC-是一系列协议的集合)
- [WebRTC（API）如何工作 #](#WebRTCAPI如何工作-)
...
`WebRTC` 协议由 `IETF` 工作组在[rtcweb](https://datatracker.ietf.org/wg/rtcweb/documents/ "rtcweb")中维护。`WebRTC` `API` 的 `W3C` 文档在[webrtc](https://www.w3.org/TR/webrtc/ "webrtc")。
...
**信令使用一种现有的协议 SDP（会话描述协议）**。SDP 是**一种纯文本协议**。每**个 SDP 消息均由键 / 值对组成**，**并包含“media sections（媒体部分）”列表。两个 WebRTC Agent 交换的 SDP 所包含一些详细信息**，如：
