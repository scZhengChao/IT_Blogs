...
- [为什么 WebRTC 需要专用的子系统进行连接？](#为什么-WebRTC-需要专用的子系统进行连接)
  - [降低带宽成本 #](#降低带宽成本-)
  - [更低延迟 #](#更低延迟-)
  - [安全的端到端通信 #](#安全的端到端通信-)
- [它是如何工作的？ #](#它是如何工作的-)
- [现实世界的网络限制 #](#现实世界的网络限制-)
  - [不在同一个网络中 #](#不在同一个网络中-)
  - [协议限制 #](#协议限制-)
  - [防火墙 /IDS 规则 #](#防火墙-IDS-规则-)
- [NAT 映射 #](#NAT-映射-)
  - [创建映射 #](#创建映射-)
  - [映射创建的行为 #](#映射创建的行为-)
    - [端点无关的映射 #](#端点无关的映射-)
    - [地址相关的映射 #](#地址相关的映射-)
    - [地址和端口相关的映射 #](#地址和端口相关的映射-)
  - [映射过滤行为 #](#映射过滤行为-)
    - [端点无关的过滤 #](#端点无关的过滤-)
    - [地址相关的过滤 #](#地址相关的过滤-)
    - [地址和端口相关的过滤 #](#地址和端口相关的过滤-)
  - [映射的刷新 #](#映射的刷新-)
- [STUN #](#STUN-)
  - [协议结构 #](#协议结构-)
    - [STUN 消息类型 #](#STUN-消息类型-)
    ...
    - [交互（Transaction）ID #](#交互TransactionID-)
    - [数据 #](#数据-)
  - [创建 NAT 映射 #](#创建-NAT-映射-)
  - [确定 NAT 类型 #](#确定-NAT-类型-)
- [TURN #](#TURN-)
  - [TURN 生命周期 #](#TURN-生命周期-)
    ...
    - [权限 #](#权限-)
    - [SendIndication/ChannelData #](#SendIndicationChannelData-)
    - [刷新 #](#刷新-)
  - [TURN 使用方法 #](#TURN-使用方法-)
    - [单个 TURN Allocation 通信 #](#单个-TURN-Allocation-通信-)
    - [双重 TURN Allocation 通信 #](#双重-TURN-Allocation-通信-)
- [ICE #](#ICE-)
  - [创建 ICE Agent #](#创建-ICE-Agent-)
  - [候选地址收集 #](#候选地址收集-)
    - [主机 #](#主机-)
    - [mDNS #](#mDNS-)
    - [服务器自反（Server Reflextive） #](#服务器自反Server-Reflextive-)
    - [Peer 自反 #](#Peer-自反-)
    - [中继 #](#中继-)
  - [连通性检查 #](#连通性检查-)
  - [候选地址选择 #](#候选地址选择-)
...
### 降低带宽成本 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#降低带宽成本 "#")

由于媒体通信直接发生在 peer 之间，因此你无需为之付费，也无需托管一个单独的服务器来转发媒体。

### 更低延迟 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#更低延迟 "#")

直接通信时速度更快！当用户必须通过你的服务器运行所有内容时，这会使传输速度变慢。

### 安全的端到端通信 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#安全的端到端通信 "#")

直接通信更安全。由于用户数据根本没有通过你的服务器，因此用户压根不需要考虑你的服务器会不会解密其数据。

## 它是如何工作的？ [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#它是如何工作的 "#")

上面描述的连接过程是通过 Interactive Connectivity Establishment（[交互式连接建立 /ICE](https://tools.ietf.org/html/rfc8445 "交互式连接建立 /ICE")） 实现的。这是另一个在 WebRTC 之前就已经出现的协议。

ICE 是一种用来寻找两个 ICE Agent 之间通信的最佳方式的协议。每个 ICE Agent 都会**发布如何访问自己的方式**，这些路径被**称为候选地址**（candidates）。**候选地址本质上是一个传输地址，** ICE Agent 认为这个传输地址可能可以被对端访问到。接下来 ICE 将确定候选地址的最佳搭配。

本章稍后将详细介绍实际的 ICE 过程。要了解 ICE 为什么存在，最好先了解我们要面临的网络特性。

## 现实世界的网络限制 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#现实世界的网络限制 "#")

ICE 就是克服现实世界网络限制的方法。在我们开始讨论 ICE 如何解决问题之前，先讨论一下有哪些实际问题。

### 不在同一个网络中 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#不在同一个网络中 "#")

在大多数情况下，两个 WebRTC Agent 不在同一个网络中。典型的呼叫通常是在没有直接连接的不同网络中的两个 WebRTC Agent 之间进行的。

下面是通过公共互联网连接的两个不同网络的示意图。在每个网络中，你拥有两个主机。

![](./image/image_ihdqLy05dk.png)
...
### 协议限制 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#协议限制 "#")

有些网络不允许 UDP 通信，或者也有可能不允许 TCP。有些网络的 MTU（Maximum Transmission Unit/ 最大传输单元）可能非常低。网络管理员可以更改许多变量，这些修改可能会使通信变得困难。

### 防火墙 /IDS 规则 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#防火墙-ids-规则 "#")
...
## NAT 映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#nat-映射 "#")
...
描述这些行为的文档是 [RFC 4787](https://tools.ietf.org/html/rfc4787 "RFC 4787")。

### 创建映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#创建映射 "#")
...
### 映射创建的行为 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#映射创建的行为 "#")

映射创建分为三类：

#### 端点无关的映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#端点无关的映射 "#")
...
#### 地址相关的映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#地址相关的映射 "#")

每次将数据包发送到新地址时，都会创建一个新的映射。如果你将两个数据包发送到不同的主机，则会创建两个映射。如果将两个数据包发送到同一远程主机，但目标端口不同，则不会创建新的映射。

#### 地址和端口相关的映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#地址和端口相关的映射 "#")

如果远程 IP 或端口不同，则会创建一个新的映射。如果将两个数据包发送到同一远程主机，但目标端口不同，则将创建一个新的映射。

### 映射过滤行为 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#映射过滤行为 "#")

映射过滤是**关于允许谁使用映射的规则**。它们分为三个类似的类别：

#### 端点无关的过滤 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#端点无关的过滤 "#")

任何人都可以使用该映射。你可以与其他多个 peer 共享该映射，他们都可以向该映射发送流量。

#### 地址相关的过滤 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#地址相关的过滤 "#")

只有**为其创建映射的主机才能使用该映射**。如果你将数据包发送到主机 `A`，则它可以根据需要响应任意数量的数据包。如果主机 `B` 尝试将数据包发送到该映射，将被忽略。

#### 地址和端口相关的过滤 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#地址和端口相关的过滤 "#")

仅有创建映射的主机和端口可以使用该映射。如果你将数据包发送到主机 `A:5000`，则它可以根据需要响应任意数量的数据包。如果主机 `A：5001` 尝试将数据包发送到该映射，将被忽略。

### 映射的刷新 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#映射的刷新 "#")
...
## STUN [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#stun "#")

**STUN（NAT 会话传输实用程序）是一种用来配合 NAT 使用的协议**。这是 WebRTC（和 ICE！）之前的另一项技术。它由[RFC 8489](https://tools.ietf.org/html/rfc8489 "RFC 8489")定义，该文件还定义了 STUN 数据包结构。STUN 协议也在 ICE/TURN 中被使用。
...
### 协议结构 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#协议结构 "#")
...
#### STUN 消息类型 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#stun-消息类型 "#")
...
#### 消息长度 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#消息长度 "#")

这就是 `Data` 段的长度。这一段中包含由`消息类型`所定义的任意数据。

#### Magic Cookie [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#magic-cookie "#")

指的是固定值 `0x2112A442`，以网络字节顺序发送。这个值有助于将 STUN 流量与其他协议区分开。

#### 交互（Transaction）ID [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#交互transactionid "#")

一个 96-bit 的标识符，用于唯一标识一个请求 / 响应对。这可以帮助你配对请求和响应。

#### 数据 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#数据 "#")
...
### 创建 NAT 映射 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#创建-nat-映射 "#")

使用 STUN 创建 NAT 映射只需要发送一个请求！你向 STUN 服务器发送一个 `STUN Binding Request`。然后，STUN 服务器回应一个 `STUN Binding Response`。 该 `STUN Binding Response` 将包含`映射地址`。`映射地址`是 STUN 服务器看到你的方式，也是你的 `NAT 映射`。 如果你希望某人向你发送数据包，那么你应该共享该`映射地址`。

人们还会将`映射地址`称为`公网 IP` 或 `Server Reflexive Candidate`。

### 确定 NAT 类型 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#确定-nat-类型 "#")
...
[RFC 5780](https://tools.ietf.org/html/rfc5780 "RFC 5780")定义了一种方法，可以运行一个测试来确定你的 NAT 类型。这很有用，因为你可能会提前知道是否可以进行直接连接。

## TURN [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#turn "#")

在无法建立直接连接的情况下，[RFC 8656](https://tools.ietf.org/html/rfc8656 "RFC 8656")中定义了 TURN（使用中继穿透 NAT）。当你的两个 peer 的 NAT 类型不兼容，或者双方使用不同协议时，就需要使用 TURN！TURN 也可以被用于保护隐私的目的。如果通过 TURN 运行所有通讯，客户的真实地址在对端是被隐藏的。
...
### TURN 生命周期 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#turn-生命周期 "#")

下面就是一个客户端创建 TURN `allocation` 时必须做的所有事情。对于其他 peer 而言，与使用 TURN 服务器的客户端进行通信和其他客户端没有任何区别，先获得 IP 和端口，然后像跟其他任何主机一样通信。

#### Allocations [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#allocations "#")
...
#### 权限 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#权限 "#")

**在你为远程主机创建权限之前，远程主机是无法通过你**的`中继传输地址`发送数据的。所谓创建权限，**即是告知 TURN 服务器一个 " 可以用来发送入站流量 " 的 IP 和端口。**
...
#### SendIndication/ChannelData [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#sendindicationchanneldata "#")
...
#### 刷新 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#刷新 "#")

Allocations 将自动销毁。要避免其过早销毁，TURN 客户端必须在创建 allocation 时指定的 `LIFETIME` 到来之前，及时刷新它们。

### TURN 使用方法 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#turn-使用方法 "#")
...
![](./image/image_LfqhHjC4vu.png)

#### 双重 TURN Allocation 通信 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#双重-turn-allocation-通信 "#")

![](https://webrtcforthecurious.com/zh/images/03-two-turn-allocations.png)

> 译注：单个 TURN Allocation 的情况，指的是一个 TURN Client 和另一个可访问的 UDP Client 的通信。双重 TURN Allocation 的情况，指的是两个 TURN Client 之间通信。

## ICE [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#ice "#")

ICE（交互式连接建立）是 `WebRTC` **连接两个** `Agent` 的方式。这也是一项 `WebRTC` 前就有的技术，在[RFC 8445](https://tools.ietf.org/html/rfc8445 "RFC 8445")中定义！ICE 是用于建立连接的协议。它会确定两个 peer 之间所有可能的路由，然后确保你保持连接状态。
...
### 创建 ICE Agent [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#创建-ice-agent "#")

ICE Agent 要么处于`控制中`，要么处于`受控中`。`控制中`的 Agent 是决定选择`候选对`的 Agent。通常来说，发送 offer 的 peer 是`控制中`的一方。

每一方都必须有一个`用户片段`和一个`密码`。必须先交换这两个值，接下来才能进行连接性检查。`用户片段`以纯文本形式发送，用于多个 ICE 会话的解复用（demux）。 `密码`用于生成 `MESSAGE-INTEGRITY` 属性。在每个 STUN 数据包的末尾，都有这个属性，该属性是使用`密码`作为密钥的整个数据包的哈希值。这用于验证数据包并确保它未被篡改。

对于 WebRTC，所有这些值都通过上一章中所述的`会话描述`进行分发。

### 候选地址收集 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#候选地址收集 "#")

现在，我们需要收集所有可能联通的地址。这些地址被称为候选地址 (Candidate)。

#### 主机 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#主机 "#")

主机候选地址直接在本地接口上侦听。可以是 UDP 或 TCP 方式。

#### mDNS [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#mdns "#")
...
#### 服务器自反（Server Reflextive） [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#服务器自反server-reflextive "#")

服务器自反候选地址是通过对 STUN 服务器执行 `STUN 绑定请求`时生成的。

当你收到 `STUN 绑定响应`时，`XOR-MAPPED-ADDRESS` 就是你的服务器自反候选地址。

#### Peer 自反 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#peer-自反 "#")
...
### 重新启动 [#](https://webrtcforthecurious.com/zh/docs/03-connecting/#重新启动 "#")
