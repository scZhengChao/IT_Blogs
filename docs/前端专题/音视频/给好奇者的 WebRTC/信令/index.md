...
- [什么是 WebRTC 信令？ #](#什么是-WebRTC-信令-)
- [WebRTC 信令如何工作？](#WebRTC-信令如何工作)
- [什么是 会话描述协议（SDP）？ #](#什么是-会话描述协议SDP-)
  - [如何阅读 SDP 信息 #](#如何阅读-SDP-信息-)
  - [WebRTC 仅使用了部分 SDP 的 key #](#WebRTC-仅使用了部分-SDP-的-key-)
  - [会话描述中的媒体描述 ](#会话描述中的媒体描述-)
  - [完整示例 #](#完整示例-)
- [会话描述协议 和 WebRTC 如何协同工作 #](#会话描述协议-和-WebRTC-如何协同工作-)
  - [什么是 Offer 和 Answer？ #](#什么是-Offer-和-Answer-)
  - [用于发送和接收的收发器（Transceivers）](#用于发送和接收的收发器Transceivers)
  - [WebRTC 用到的 SDP 值 #](#WebRTC-用到的-SDP-值-)
    - [group:BUNDLE #](#groupBUNDLE-)
    - [fingerprint:sha-256 #](#fingerprintsha-256-)
    - [setup: #](#setup-)
    - [mid #](#mid-)
    - [ice-ufrag #](#ice-ufrag-)
    - [ice-pwd #](#ice-pwd-)
    ...
    - [candidate #](#candidate-)
    - [ssrc #](#ssrc-)
  - [WebRTC 会话描述示例 #](#WebRTC-会话描述示例-)
  - [进一步的话题 #](#进一步的话题-)

## 什么是 WebRTC 信令？ [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#什么是-webrtc-信令 "#")
...
## 什么是 *会话描述协议*（SDP）？ [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#什么是-会话描述协议sdp "#")

会话描述**协议定义于 **[**RFC 8866**](https://tools.ietf.org/html/rfc8866 "RFC 8866")** 中。它是一个 key/value 协议，每一行是一个值**。看起来类似于 INI 文件。\*\* 一个会话描述包含零个或多个媒体描述。对此模型，可以理解为会话描述包含了一个媒体描述的数组\*\*。

一个媒体描述通常映射到单个媒体流。因此，如果你想描述一个包含三个视频流和两个音轨的呼叫，需要五个媒体描述。

### 如何阅读 SDP 信息 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#如何阅读-sdp-信息 "#")
...
### WebRTC 仅使用了部分 SDP 的 key [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#webrtc-仅使用了部分-sdp-的-key "#")

WebRTC 并未使用会话描述协议定义的所有 key，只有那些在 `JavaScript Session Establishment Protocol` (JSEP，协议定义在[RFC 8829](https://datatracker.ietf.org/doc/html/rfc8829 "RFC 8829")) 中被使用到的 key 是重要的。你**当前只需要理解下面的 7 个 key。**
...
### 完整示例 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#完整示例 "#")
...
## *会话描述协议* 和 WebRTC 如何协同工作 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#会话描述协议-和-webrtc-如何协同工作 "#")

下一块拼图是理解 WebRTC \_ 如何 \_ 使用会话描述协议。

### 什么是 Offer 和 Answer？ [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#什么是-offer-和-answer "#")

WebRTC 使用 Offer/Answer 模型。这指的是，一个 WebRTC Agent 发出 “Offer” 以开始呼叫，如果另一个 WebRTC Agent 愿意接受 “Offer” 的内容，它会响应 “Answer”。

这使得应答**者有机会拒绝媒体描述中的某些不支持的编解码器**，也是两个 `peer` 互相**理解他们希望交换何种格式的方式。**

### 用于发送和接收的收发器（Transceivers）

**收发器是** `WebRTC` 中**特有的概念**，你将在 `API` 中看到它。它的作用是将 " 媒体描述 " 暴露给 `JavaScript API`。每个媒体描述都将成为一个收发器。每次创建收发器时，都会将**新的媒体描述添加**到**本地会话描述中。**
...
### WebRTC 用到的 SDP 值 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#webrtc-用到的-sdp-值 "#")

这个列表包含了你将在 WebRTC Agent 的会话描述中看到的一些常见属性。这些值控制着我们尚未讨论到的子系统。

#### `group:BUNDLE` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#groupbundle "#")

BUNDLE 是一种在单个连接上传输多种类型流量的行为。一些 WebRTC 实现对每个媒体流会使用专用的连接。但 BUNDLE 方式应该是首选。

#### `fingerprint:sha-256` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#fingerprintsha-256 "#")

该属性是 `peer` 用于 `DTLS` **证书的哈希值**。`DTLS` 握手完成后，你可以将**其与实际证书进行比较，以确认你正在与预期的对象进行通信**。

> 译注：下面是[RFC 4572](https://tools.ietf.org/html/rfc4572 "RFC 4572")中的一个例子
> a=fingerprint:SHA-1 \\
> 4A:AD:B9:B1:3F:82:18:3B:54:02:12:DF:3E:5D:49:6B:19:E5:7C:AB

#### `setup:` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#setup "#")
...
#### `mid` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#mid "#")

该属性是每个 Media Description 的唯一 ID。用于标识媒体。

#### `ice-ufrag` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#ice-ufrag "#")

该属性是 ICE Agent 的用户片段值。用于 ICE 流量的身份验证。

#### `ice-pwd` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#ice-pwd "#")

该属性是 ICE Agent 的密码。用于 ICE 流量的身份验证。

#### `rtpmap` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#rtpmap "#")

该属性用于将特定的编解码器映射到 RTP 有效负载类型。有效负载类型不是静态的，因此对于每次呼叫，发起者都需要确定每个编解码器的有效负载类型。

#### `fmtp` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#fmtp "#")

该属性为一种有效负载类型定义附加的值。要传递特定的视频配置文件或编码器设置时，这很有用。

#### `candidate` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#candidate "#")

该属性是来自 ICE Agent 的 ICE 候选地址。这是一个可能被 WebRTC Agent 使用的地址。这些将在下一章中详细说明。

#### `ssrc` [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#ssrc "#")

一个同步源（SSRC）定义了一个单独的媒体流。

`label` 是此媒体流的 ID。`mslabel` 是容器的 ID，该容器中可以有多个流。

### WebRTC 会话描述示例 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#webrtc-会话描述示例 "#")
...
### 进一步的话题 [#](https://webrtcforthecurious.com/zh/docs/02-signaling/#进一步的话题 "#")
