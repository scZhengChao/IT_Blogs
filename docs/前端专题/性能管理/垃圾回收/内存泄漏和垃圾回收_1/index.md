...
![  ](../assets/内存泄漏和垃圾回收_1/image/640_Q8craQJt98.png "  ")

1. 直接将标记为垃圾的数据清理掉。
2. 多次标记-清除后，会产生大量不连续的内存碎片，**需要进行内存整理。**

![  ](../assets/内存泄漏和垃圾回收_1/image/640_FCjnqen9zc.png "  ")

### **副垃圾回收器**负责**新生代的垃圾回收，通常只支持 1\~8 M 的容量**。

新生代被分为两个区域：一**般是对象区域，一半是空闲区域**。

![  ](../assets/内存泄漏和垃圾回收_1/image/640_JVUG3K5Mpe.png "  ")
