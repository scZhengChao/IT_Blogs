...
- [
  ...
  &#x20;               ](#------------------------------------------------------------完善的前端异常监控解决方案----------------)
- [1 前端异常](#1-前端异常)
  - [1.1 前端异常分类](#11-前端异常分类)
  - [1.2 异常错误原因分类](#12-异常错误原因分类)
- [2 异常采集](#2-异常采集)
  - [2.1 采集内容](#21-采集内容)
  - [2.2 异常捕获](#22-异常捕获)
  - [2.3 跨域脚本异常](#23-跨域脚本异常)
  - [2.4 异常录制](#24-异常录制)
  - [2.5 异常级别](#25-异常级别)
- [3 整理与上报方案](#3-整理与上报方案)
  - [3.1 前端存储日志](#31-前端存储日志)
  - [3.2 前端整理日志](#32-前端整理日志)
  - [3.3 上报日志](#33-上报日志)
    - [a. 即时上报](#a-即时上报)
    - [b. 批量上报](#b-批量上报)
    - [c. 区块上报](#c-区块上报)
    - [d. 用户主动提交](#d-用户主动提交)
  - [3.4 压缩上报数据](#34-压缩上报数据)
- [4 日志接收与存储](#4-日志接收与存储)
  - [4.1 接入层与消息队列](#41-接入层与消息队列)
  - [4.2 日志存储系统](#42-日志存储系统)
  - [4.3 搜索](#43-搜索)
- [5 日志统计与分析](#5-日志统计与分析)
  - [5.1 用户纬度](#51-用户纬度)
  - [5.2 时间维度](#52-时间维度)
  - [5.3 性能维度](#53-性能维度)
  - [5.4 运行环境维度](#54-运行环境维度)
  - [5.5 细粒度代码追踪](#55细粒度代码追踪)
  - [5.6 场景回溯](#56-场景回溯)
- [6 监控与通知](#6-监控与通知)
  - [6.1 自定义触发条件的告警](#61-自定义触发条件的告警)
  - [6.2 推送渠道](#62-推送渠道)
  - [6.3 推送频率](#63-推送频率)
  - [6.4 自动报表](#64-自动报表)
  - [6.5 自动产生bug工单](#65-自动产生bug工单)
- [7 修复异常](#7-修复异常)
  - [7.1 sourcemap](#71-sourcemap)
  - [7.2 从告警到预警](#72-从告警到预警)
  - [7.3 智能修复](#73-智能修复)
- [8 异常测试](#8-异常测试)
  - [8.1 主动异常测试](#81-主动异常测试)
  - [8.2 随机异常测试](#82-随机异常测试)
- [9 部署](#9-部署)
  - [9.1 多客户端](#91-多客户端)
  - [9.2 集成便捷性](#92-集成便捷性)
  - [9.3 管理系统的可扩展](#93-管理系统的可扩展)
  - [9.4 日志系统权限](#94-日志系统权限)
- [10 其他](#10-其他)
  - [10.1 性能监控](#101-性能监控)
  - [10.2 API Monitor](#102-API-Monitor)
  - [10.3 数据脱敏](#103-数据脱敏)
- [结语](#结语)
...
、性能监控等，本文主要讨论异常监控。对于前端而言，和后端处于同一个监控系统中，前端有自己的监控方案，后端也有自己等监控方案，但两者并不分离，因为一个用户在操作应用过程中如果出现异常，有可能是前端引起，也有可能是后端引起，需要有一个机制，将前后端串联起来，使监控本身统一于监控系统。因此，即使只讨论前端异常监控，其实也不能严格区分前后端界限，而要根据实际系统的设计，在最终的报表中体现出监控对开发和业务的帮助。

一般而言，一个监控系统，大致可以分为四个阶段：日志采集、日志存储、统计与分析、报告和警告。

![  ](./assets/image/640_CspCQlwbz9.webp "  ")
...
![  ](./assets/image/640_qbA9H13-SX.jpeg "  ")
...
| 原因     | 案例                                                                                  | 频率 |
| ------ | ----------------------------------------------------------------------------------- | -- |
| 逻辑错误   | 1)    业务逻辑判断条件错误2)    事件绑定顺序错误3)    调用栈时序错误4)    错误的操作js对象                          | 经常 |
| 数据类型错误 | 1)    将null视作对象读取property2)    将undefined视作数组进行遍历3)    将字符串形式的数字直接用于加运算4)    函数参数未传 | 经常 |
| 语法句法错误 |                                                                                     | 较少 |
| 网络错误   | 1)    慢2)    服务端未返回数据但仍200，前端按正常进行数据遍历3)    提交数据时网络中断4)    服务端500错误时前端未做任何错误处理      | 偶尔 |
| 系统错误   | 1)    内存不够用2)    磁盘塞满3)    壳不支持API4)    不兼容                                         | 较少 |
...
![  ](./assets/image/640_sM06SRTlTB.webp "  ")
...
![  ](./assets/image/640_U4lWg01WQu.webp "  ")

是来自阿里的一套异常录制还原方案示意图，用户在界面上的操作产生的events和mutation被产品收集起来，上传到服务器，经过队列处理按顺序存放到数据库中。当需要进行异常重现的时候，将这些记录从数据库中取出，采用一定的技术方案，顺序播放这些记录，即可实现异常还原。
...
![  ](./assets/image/640_5Ivj8EUfsm.webp "  ")
...
![  ](./assets/image/640_JRy4_FmKN4.jpeg "  ")
...
![  ](./assets/image/640_pwRdIV7DOw.webp "  ")
...
![  ](./assets/image/640_oLRv5kPL1C.jpeg "  ")
...
![  ](./assets/image/640_iQvXmMygtS.jpeg "  ")
...
![  ](./assets/image/640_HLG0y6P7ln.jpeg "  ")
...
[听说你不知道如何监控 Node 服务的内存？](http://mp.weixin.qq.com/s?__biz=MzAxODE2MjM1MA==\&mid=2651559956\&idx=2\&sn=9a7868094dc36c09f73e2dcaa0f1b31a\&chksm=80254fd5b752c6c388dfaf55a726746238875a18ac76bc2a9ae6954f38d8fbba724811be13a8\&scene=21#wechat_redirect "听说你不知道如何监控 Node 服务的内存？")

2、

[如何进行 web 性能监控](http://mp.weixin.qq.com/s?__biz=MzAxODE2MjM1MA==\&mid=2651557717\&idx=1\&sn=0da018e3e7b3b7fe065673f7f4e6f0f1\&chksm=80255894b752d1823dd0c1c75199e842511ebb82abcf6426ecfaa8b8409f6b92e1ea2ee74845\&scene=21#wechat_redirect "如何进行 web 性能监控")

3、

[网易云音乐前端性能监控实践](http://mp.weixin.qq.com/s?__biz=MzAxODE2MjM1MA==\&mid=2651557495\&idx=1\&sn=54215ea99470fed0f1457476435fe98b\&chksm=802559b6b752d0a096d0512f57802afbe0aa31c7d62e68a07de3fd1bf1cd0265def7f23b48b2\&scene=21#wechat_redirect "网易云音乐前端性能监控实践")

觉得本文对你有帮助？请分享给更多人

关注「前端大全」加星标，提升前端技能

![  ](./assets/image/640_A2RPs5sfhL.jpeg "  ")
