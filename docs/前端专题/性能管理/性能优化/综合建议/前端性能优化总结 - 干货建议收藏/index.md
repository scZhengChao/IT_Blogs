...
- [加载时性能优化](#加载时性能优化)
  ...
  - [2.使用HTTP2](#2使用HTTP2)
    ...
    - [首部压缩](#首部压缩)
    - [服务器推送](#服务器推送)
  - [3.减少HTTP请求数量](#3减少HTTP请求数量)
  - [4.压缩、合并文件](#4压缩合并文件)
    - [提取公共代码](#提取公共代码)
  - [5.采用svg图片或者字体图标](#5采用svg图片或者字体图标)
  - [6.按需加载代码，减少冗余代码](#6按需加载代码减少冗余代码)
    - [按需加载](#按需加载)
    - [减少冗余代码](#减少冗余代码)
  - [7.服务器端渲染](#7服务器端渲染)
  - [8. 使用 Defer 加载JS](#8-使用-Defer-加载JS)
  - [9. 静态资源使用 CDN](#9-静态资源使用-CDN)
  - [10. 图片优化](#10-图片优化)
    - [雪碧图 ](#雪碧图-)
    - [图片懒加载 ](#图片懒加载-)
    - [css中图片懒加载 ](#css中图片懒加载-)
- [运行时性能优化](#运行时性能优化)
  - [1. 减少重绘与重排](#1-减少重绘与重排)
    - [重排](#重排)
    - [重绘](#重绘)
    - [1.1 避免table布局 ](#11-避免table布局-)
    - [1.2 分离读写操作 ](#12-分离读写操作-)
    - [1.3 样式集中改变 ](#13-样式集中改变-)
    - [1.4 position属性为absolute或fixed ](#14-position属性为absolute或fixed-)
  - [2. 避免页面卡顿](#2-避免页面卡顿)
  - [3. 长列表优化](#3-长列表优化)
    - [1.1 实现虚拟列表 ](#11-实现虚拟列表-)
  - [4. 滚动事件性能优化](#4-滚动事件性能优化)
  - [5. 使用 Web Workers](#5-使用-Web-Workers)
  - [6. 写代码时的优化点](#6-写代码时的优化点)
    - [6.1 使用事件委托](#61-使用事件委托)
    - [6.2 if-else 对比 switch](#62-if-else-对比-switch)
    - [6.3 布局上使用flexbox](#63-布局上使用flexbox)

**为什么要做性能优化？性能优化到底有多重要？**

 网站的性能优化对于用户的留存率、转化率有很大的影响，所以对于前端开发来说**性能优化能力**也是重要的考察点。  &#x20;
...
       DNS 作为互联网的基础协议，其解析的速度似乎容易被网站优化人员忽视。现在大多数新浏览器已经针对DNS解析进行了优化，典型的一次DNS解析耗费

20-120毫秒，减少DNS解析时间和次数是个很好的优化方式。DNS Prefetching是具有此属性的域名不需要用户点击链接就在后台解析，而域名解析和内容载入是串行的网络操作，所以这个方式能减少用户的等待时间，提升用户体验。&#x20;
...
       在开发SPA项目时，项目中经常存在十几个甚至更多的路由页面， 如果将这些页面都打包进一个JS文件， 虽然减少了HTTP请求数量， 但是会导致文件比较大，同时加载了大量首页不需要的代码，有些得不偿失，
...
babel-loader用 include 或 exclude 来帮我们避免不必要的转译，不转译node\_moudules中的js文件,其次在缓存当前转译的js文件，设置loader: 'babel-loader?cacheDirectory=true'
...
&#x20;这里的 @babel/runtime包就声明了所有需要用到的帮助函数，而 @babel/plugin-transform-runtime 的作用就是将所有需要 helper 函数的文件，从 @babel/runtime包 引进来：&#x20;
...
只要给 script 标签加上 defer 属性就可以了，异步下载，延迟执行。
...
       一般来说，我们访问网站页面时，其实很多图片并不在首屏中，如果我们都加载的话，相当于是加载了用户不一定会看到图片， 这显然是一种浪费。解决的核心思路就是

**懒加载**：实现方式就是先不给图片设置路径，当图片出现在浏览器可视区域时才设置真正的图片路径。&#x20;
...
如果想使用懒加载，还可以借助一些已有的工具库，

例如 aFarkas/lazysizes、verlok/lazyload、tuupola/lazyload 等。&#x20;
...
DOM 的多个读操作（或多个写操作），应该放在一起。不要两个读操作之间，加入一个写操作。 （每次读都要获取一次最新的）
...
       我们目前大多数屏幕的刷新率-60次/s,浏览器渲染更新页面的标准帧率也为60次/s --**60FPS(frames/pre second)**, 那

么每一帧的预算时间约为16.6ms ≈ 1s/60，浏览器在这个时间内要完成所有的整理工作，如果无法符合此预算， 帧率将下降，内容会在屏幕抖动， 此现象通常称为**卡顿**。&#x20;
...
layout，再把它画到内存的一个画布里面，paint成了像素，最后把这个画布刷到屏幕上去，叫做composite，形成一帧。
...
对节流和防抖不太了解的可以看这篇文章：[**老生常谈的防抖与节流**](https://mp.weixin.qq.com/s?__biz=MzI4NTQ2Nzk2MA==\&mid=2247483779\&idx=1\&sn=7b442727d1671420655bbc1c5bcb54b8\&scene=21#wechat_redirect "老生常谈的防抖与节流")
