...
- [性能面板](#性能面板)
  - [Network](#Network)
  - [Main](#Main)
  - [Compositor & Raster](#Compositor--Raster)
  - [GPU](#GPU)
- [浏览器的工作报告](#浏览器的工作报告)
  - [文档的下载解析](#文档的下载解析)
  - [不同资源的处理](#不同资源的处理)
  - [脚本的解析执行](#脚本的解析执行)
  - [lifecycle 和 paint timing](#lifecycle-和-paint-timing)
    - [beforeunload](#beforeunload)
    - [pagehide](#pagehide)
    - [first paint](#first-paint)
    - [DOMContentLoaded](#DOMContentLoaded)
    - [pageshow/load](#pageshowload)
  - [任务与性能问题](#任务与性能问题)
...
![](./assets/image/image_-7cG5eEL7H.webp)

![](https://mmbiz.qpic.cn/sz_mmbiz_png/H8M5QJDxMHr1qBlVQgmiaVxbW0dhaiaLo916MbdvRPyGBpo8IlLAibwxCdJtRCMS83MZrsgSgzTu205uDC0RMNgzA/640?wx_fmt=png\&tp=webp\&wxfrom=5\&wx_lazy=1)

由下图我们可以看到性能面板呈现的几个主要线程。**性能面板并不包含架构中全部的线程**，主要还是与页面渲染过程相关的部分。 &#x20;

![](./assets/image/image_oC8j3wdGig.webp)

### Network

Network 代表浏览器进程中的网络线程，我们可以看到时间轴上包含了所有的网络请求和文件下载的调用信息，并以不同颜色标识不同类型的资源。 &#x20;

![](https://mmbiz.qpic.cn/sz_mmbiz_png/H8M5QJDxMHr1qBlVQgmiaVxbW0dhaiaLo9ROyMWqiaCXSATfYd5xONdQlfYB2YK8Joeyvibs8D2ibFfco8uEYLYpJvA/640?wx_fmt=png\&tp=webp\&wxfrom=5\&wx_lazy=1)

### Main

Main 代表渲染进程中的主线程，**渲染相关的事情基本都是它来做，脚本执行、样式计算、布局计算、绘制**等等。

![](./assets/image/image_gQYJvRceb5.webp)

### Compositor & Raster

**Compositor 代表渲染进程中的合成线程**，**Raster 代表渲染进程中的栅格线程**。如今浏览器绘制一个页面，可以分为以下几步：

- 主线程**将页面分成若干图层**（后文中会提及 Update Layer Tree）
- 栅格线程分别对**每一个层进行栅格化处理**
- **合成线程将栅格化的图块合并成一个页面**

![](./assets/image/image_HuebIW7quH.webp)

我们可以看到，在性能面板中主线程在最后调用**了栅格线程做实际的渲染**

![](./assets/image/image_9ZcB-yRfpy.webp)
...
![](./assets/image/image_ZkTYK5kDy6.webp)

HTML 文档下载完成后，浏览器开始按照 `HTML` 标准对 `index.html` 进行解析，在主线程中将接收**到的文本字符串解析为 DOM** 。我们可以注意到，HTML 的解析过程并不是一气呵成，这是因为 HTML 通常还包括了其他外部资源，如图片、CSS、JS 等。这些文件需要通过网络请求或缓存来获取。其中，**当 HTML 解析器解析到**\*\*`<script>`\*\***标签时，HTML 文档的解析过程就会中止，转而去加载、解析和执行脚本**。因此，从主线程的时间轴可以看出，Parse HTML 的过程是断断续续的。

![](./assets/image/image_3JGj3n9QB8.webp)
...
![](./assets/image/image_DWAyRN3ldB.webp)
...
```javascript title="let script = document.createElement('script');script.src = %22/xxx/a.js%22;document.body.append(script);"
...
![](./assets/image/image_1Uo2qLyiac.webp)

![](./assets/image/image_sD1EOCeBq3.webp)

### lifecycle 和 paint timing

下图展示的是文章中提及的页面生命周期流程图。本节我们结合 Performance，对照该图进行观察。

![](./assets/image/image_H21yRtle0e.webp)
...
![](./assets/image/image_7tNCqJY50D.webp)
...
![](./assets/image/image_IHzbX1PYTT.webp)
...
- first paint：指的是**首个像素开始绘制到屏幕上的时机**，例如一个页面的背景色
- first contentful paint：指的是**开始绘制内容的时机，如文字或图片**

![](./assets/image/image_sD17TPAxrC.webp)
...
![](./assets/image/image_leHEJ9iiIQ.webp)

**Layout 之后的过程很快，这里放大些倍数来查看：**

![](./assets/image/image_Ysnc1lAr0P.webp)

#### DOMContentLoaded

DOMContentLoaded 表示\*\* HTML 已经完全被加载和解析\*\*，当然**样式表、图片等资源还不一定已经完成加载**。从下图中可以看到，经过多段 HTML 解析后，DCL 之后就没有其他的 Parse HTML 了。

![](./assets/image/image_ikj_QDmBlQ.webp)
...
![](./assets/image/image_bFT6UYAWav.webp)

### 任务与性能问题

比较可惜的是，`Performance` 还无法清晰的看出 `Event Loop`。下图中**灰色的 Task **并不是指宏任务，其代表的是「**当前主线程忙碌，无法响应用户交互**」；`Run Microtasks` 则确实是在**一次任务的末尾执行的微任务**。当我们点开调用栈观察时，可以看到源码中的回调函数以及对应的源码位置。

![](./assets/image/image_8exJ9AvyHa.webp)

通过 Task 可以定位性能出现问题的地方。RAIL 模型告诉我们需要重点关注占用 CPU 超出 50ms 的复杂任务，以提供连贯的交互体验。当然，这里更多的是对交互阶段的响应的要求，而不一定是对初始加载阶段的要求。

![](./assets/image/image_ZYBeVKSvki.webp)
