...
![](image_4GtMeDWFzc.png)

赋值操作改变执行上下文中的值

![](image_0k0K61lZaV.png)

**第二步是调用 addAll 函数。**

执行 addAll 函数时的调用栈

![](image_fZj-0L7yr_.png)

**第三步，当执行到 add 函数**

执行 add 函数时的调用栈

![](image_ehBueUWgv9.png)

当 add 函数返回时，该函数的执行上下文就会从**栈顶弹出**

![](image_9eSbrLxM-u.png)

addAll 的执行上下文也会从栈顶部弹出，此时调用栈中就只剩下全局上下文。

![](image_bbxvAailTq.png)
