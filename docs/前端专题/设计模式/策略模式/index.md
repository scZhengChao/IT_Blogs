...
- [实现和使用分离](#实现和使用分离)
  ...
  - [实例代码实现](#实例代码实现)
  - [策略模式的通用实现](#策略模式的通用实现)
  - [4. 实战中的策略模式](#4-实战中的策略模式)
    - [4.1 表格 formatter](#41-表格-formatter)
      - [首先实现文件计算的算法：](#首先实现文件计算的算法)
    - [4.2 表单验证](#42-表单验证)
  - [5. 策略模式的优缺点](#5-策略模式的优缺点)
  - [6. 策略模式的适用场景](#6-策略模式的适用场景)
  - [7. 其他相关模式](#7-其他相关模式)
    - [7.1 策略模式和模板方法模式](#71-策略模式和模板方法模式)
    - [7.2 策略模式和享元模式](#72-策略模式和享元模式)

# **策略模式**

 （Strategy Pattern）又称政策模式，其定义一系列的算法，把它们一个个封装起来，并且使它们可以互相替换。封装的策略算法一般是独立的，策略模式根据输入来调整采用哪个算法。关键是策略的
...
通过判断输入的折扣类型来计算计算商品总价的方式，几个 if-else就满足了需求，但是这样的做法的缺点也很明显：
...
我们可以改造一下，将计算折扣的**算法部分提取出来保存为一个对象**，折扣的**类型作为 key**，这样索引的时候
...
![  ](./image/4c97f7ce24291da8bb2fe400fd8eab87_sAJ1qAPTzu.png "  ")
...
Element 的表格控件的 Column 接受一个 formatter参数，用来格式化内容，其类型为函数，并且还可以接受几个特定参数，像这样：
...
```javascript 
 exportconst StrategyMap = { 
      /* Strategy 1: 将文件大小（bit）转化为 KB */ 
      bitToKB: val => { 
           const num = Number(val) 
           returnisNaN(num) ? val : (num / 1024).toFixed(0) + 'KB' 
      }, 
      /* Strategy 2: 将文件大小（bit）转化为 MB */ 
      bitToMB: val => { 
           const num = Number(val) 
           returnisNaN(num) ? val : (num / 1024 / 1024).toFixed(1) + 'MB' 
      } 
 } 
...
![  ](./image/dfc49573bb2445f014b8acfbb50bbf38_bWWew0Kstt.png "  ")
...
 函数中，但是这样就不好复用使用频率比较高的表单验证方法了，这时我们可以结合策略模式和函数柯里化的知识来重构一下。首先我们在项目的工具模块（一般是 utils文件夹）实现通用的表单验证方法：
...
然后在 utils/index.js中增加一个柯里化方法，用来生成表单验证函数：
...
```javascript 
 ...
 /* 生成表格自定义校验函数 */ 
 expor tconst formValidateGene = (key, msg) =>(rule, value, cb) => { 
...
上面的 formValidateGene函数接受两个参数，第一个是验证规则，也就是 src/utils/validates.js
...
![  ](./image/ae1bbe1fbf31561931ccc8b44dadc2a5_fFbLJn3DIH.png "  ")
