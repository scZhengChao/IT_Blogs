...
- [一、什么是 Tree Shaking](#一什么是-Tree-Shaking)
- [1.1 在 Webpack 中启动 Tree Shaking](#11-在-Webpack-中启动-Tree-Shaking)
- [1.2 理论基础](#12-理论基础)
- [1.3 示例](#13-示例)
- [二、实现原理](#二实现原理)
- [2.1 收集模块导出](#21-收集模块导出)
- [2.2 标记模块导出](#22-标记模块导出)
- [2.3 生成代码](#23-生成代码)
- [2.4 删除 Dead Code](#24-删除-Dead-Code)
- [2.5 总结](#25-总结)
- [三、最佳实践](#三最佳实践)
- [3.1 避免无意义的赋值](#31-避免无意义的赋值)
- [3.3 使用 #pure 标注纯函数调用](#33-使用-pure-标注纯函数调用)
- [3.3 禁止 Babel 转译模块导入导出语句](#33-禁止-Babel-转译模块导入导出语句)
- [3.4 优化导出值的粒度](#34-优化导出值的粒度)
- [3.5 使用支持 Tree Shaking 的包](#35-使用支持-Tree-Shaking-的包)
...
Tree-Shaking 是一种基于` ES Module` 规范的 `Dead Code Elimination` 技术，它会在运行过程中**静态分析模块之间的导入导出**，确定 `ESM` 模块中**哪些导出值未曾其它模块使用，并将其删除**，以此实现打包产物的优化。
...
而 ESM 方案则从规范层面规避这一行为，它要求所有的**导入导出语句只能出现在模块顶层**，且**导入导出的模块名必须为字符串常量**，这意味着下述代码在 ESM 方案下是非法的：
...
所以，`ESM` 下模块之间的**依赖关系是高度确定**的，**与运行状态无关**，编译工具只需要对 `ESM` **模块做静态分析**，就可以**从代码字面量中推断出哪些模块值未曾被其它模块使用，这是实现 ****`Tree Shaking`**** 技术的****必要条件****。**
...
Webpack 中，Tree-shaking 的实现一是先\*\*「标记」**出模块导出值中哪些没有被用过，二是使用 `Terser`** 删掉\*\*这些没被用到的导出语句。标记过程大致可划分为三个步骤：

- `Make` 阶段，收集**模块导出变量**并**记录到模块依赖关系图** `ModuleGraph` 变量中
...
注意，这个时候 `foo` 变量对应的代码 `const foo='foo'` 都还保留完整，这是因为**标记功能只会影响到模块的导出语句**，真正执行“**「Shaking」”操作的是 ****`Terser`**** 插件**。例如在上例中 `foo` 变量经过标记后，已经变成一段 Dead Code\*\* —— 不可能被执行到的代码\*\*，这个时候只需要用 `Terser` 提供的 `DCE`\*\* 功能就可以删除这一段定义语句\*\*，以此实现完整的 `Tree Shaking` 效果。
...
> ❝ 关于 Make 阶段的更多说明，请参考前文 [\[万字总结\] 一文吃透 Webpack 核心原理](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/SbJNbSVzSPSKBe2YStn2Zw "\[万字总结] 一文吃透 Webpack 核心原理") 。
> ❞
...
1. [\[万字总结\] 一文吃透 Webpack 核心原理](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/SbJNbSVzSPSKBe2YStn2Zw "\[万字总结] 一文吃透 Webpack 核心原理")
2. [有点难的 webpack 知识点：Dependency Graph 深度解析](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/kr73Epnn6wAx9DH7KRVUaA "有点难的 webpack 知识点：Dependency Graph 深度解析")

## **2.2 标记模块导出**

模块导出信息收集完毕后，`Webpack` 需要标记出各个模块的导出列表中，**哪些导出值有被其它模块用到，哪些没有，这**一过程发生在 `Seal` 阶段，主流程：
...
> ❝ 关于 Webpack 产物的内容及 `__webpack_require__.d` 方法的含义，可参考 [Webpack 原理系列六： 彻底理解 Webpack 运行时](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/nkBvbwpzeb0fzG02HXta8A "Webpack 原理系列六： 彻底理解 Webpack 运行时") 一文。
> ❞
...
1. [\[万字总结\] 一文吃透 Webpack 核心原理](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/SbJNbSVzSPSKBe2YStn2Zw "\[万字总结] 一文吃透 Webpack 核心原理")
2. [有点难的 webpack 知识点：Dependency Graph 深度解析](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/kr73Epnn6wAx9DH7KRVUaA "有点难的 webpack 知识点：Dependency Graph 深度解析")
3. [Webpack 原理系列六： 彻底理解 Webpack 运行时](https://link.zhihu.com/?target=https://mp.weixin.qq.com/s/nkBvbwpzeb0fzG02HXta8A "Webpack 原理系列六： 彻底理解 Webpack 运行时")

❞

## **三、最佳实践**

虽然 `Webpack` 自 `2.x `开始就原生支持 Tree Shaking 功能，但受限于 JS 的动态特性与模块的复杂性，**直至最新的 5.0 版本依然没有解决许多代码副作用带来的问题，使得优化效果并不如 Tree Shaking 原本设想的那么完美，所以需要****使用者有意识地优化代码结构****，或使用一些****补丁技术帮助 Webpack 更精确地检测无效代码****，完成 Tree Shaking 操作。**

## **3.1 避免无意义的赋值**

使用 Webpack 时，需要有意识规避一些不必要的赋值操作，观察下面这段示例代码：

![](./image/image_Bgg3VK0XgR.png)
...
![](./image/image_IJ8Ut28ecO.png)

示例中，`foo('be retained')` 调用没有带上 `/*#__PURE__*/` 备注，代码被保留；作为对比，`foo('be removed')` 带上 Pure 声明后则被 Tree Shaking 删除。

## **3.3 禁止 Babel 转译模块导入导出语句**

`Babel` 是一个非常流行的 `JavaScript` 代码转换器，它能够将**高版本的 JS 代码等价转译为兼容性更佳的低版本代码**，使得**前端开发者能够使用最新的语言特性开发出兼容旧版本浏览器的代码。**
...
即使**实际上**只用到 `default` **导出值的其中一个属性**，整个 `default` **对象依然会被完整保留**。所以实际开发中，应该尽量保持**导出值颗粒度和原子性**，上例代码的优化版本：
...
如果可以的话，应**尽量使用支持** `Tree Shaking` 的 `npm` 包，例如：
