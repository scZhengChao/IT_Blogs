...
- [使用静态代码分析工具unimported：](#使用静态代码分析工具unimported)
- [使用静态分析工具（如 ESLint）：](#使用静态分析工具如-ESLint)
- [使用第三方工具：](#使用第三方工具)
  - [depcheck](#depcheck)
  - [webpack-unused](#webpack-unused)

要找到 [React](https://so.csdn.net/so/search?q=React\&spm=1001.2101.3001.7020 "React") 项目中所有未被引用的文件，可以使用工具来进行静态代码分析。以下是一些方法：

# **使用静态代码分析工具unimported：**

[静态代码分析工具](https://so.csdn.net/so/search?q=静态代码分析工具\&spm=1001.2101.3001.7020 "静态代码分析工具")可以找到未被引用的 JSX 文件。一个常用的工具是 “unimported”。以下是使用它的步骤：
...
- 在[项目](https://so.csdn.net/so/search?q=项目\&spm=1001.2101.3001.7020 "项目")根目录中运行以下命令，以查找未被引用的 JSX 文件：
...
- “unimported” 将列出src中未被引用的 [JSX](https://so.csdn.net/so/search?q=JSX\&spm=1001.2101.3001.7020 "JSX") 文件。

# **使用静态分析工具（如 ESLint）：**

1. ESLint 是一个流行的 JavaScript 代码检查工具，可以找到未被引用的变量和文件。配置 ESLint 来查找未被引用的文件，并在输出中列出它们。以下是一些步骤：
...
1. 还可以使用第三方工具，如 `depcheck` 或 `webpack-unused`，这些工具专门用于查找未被引用的文件和依赖。它们可以扫描项目文件并识别未被引用的文件。
   > 使用 `depcheck`
...
[静态资源分析（不靠谱）](./静态资源分析（不靠谱）/index.md "静态资源分析（不靠谱）")

[useless-files-webpack-plugin](./useless-files-webpack-plugin/index.md "useless-files-webpack-plugin")
