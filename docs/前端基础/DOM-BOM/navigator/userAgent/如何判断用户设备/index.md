...
- [使用用户代理字符串（User-Agent）](#使用用户代理字符串User-Agent)
- [使用视口尺寸](#使用视口尺寸)
- [使用 CSS 媒体查询](#使用-CSS-媒体查询)
- [注意](#注意)
...
- **用户代理字符串被视为不可靠**：由于**用户代理字符串可以被修改，某些情况下可能不能准确反映用户的设备信息。**
- **响应式设计原则**：在进行设备检测时，最佳实践是**根据内容和功能的需要来适应不同设备，而不是针对特定设备进行优化或限制。**
