...
- [JavaScript 异步原理](#JavaScript-异步原理)
  - [事件循环](#事件循环)
    - [任务类型](#任务类型)
    - [事件循环流程图](#事件循环流程图)
...
- [浏览器与 Nodejs 中的异同](#浏览器与-Nodejs-中的异同)
- [异步编程的 BadCase](#异步编程的-BadCase)
  - [异步函数串行执行](#异步函数串行执行)
  - [无法捕获错误](#无法捕获错误)
  - [同步思维编写异步代码](#同步思维编写异步代码)
...
> 当然，在浏览器上还是有可以开启多个线程的解决方案 **Web Worker**，但是它**只能执行计算类的操作，无法操作 DOM**。
...
> 回调函数**非常简单容易理解和实现，缺点不利于代码的维护和阅读，各个部分之间高度耦合，还会造成回调地狱。**
...
> Promise 是为了解决回调地狱才被提出来的，它允许将**传统的嵌套回调函数写法转化为链式调用。**
...
> Generator 函数可以**暂停执行和恢复执行**，同时它还具备两个特性：函数体内的**数据转换和错误处理机制**。相信很多同学在实际工作中，很少用到 generator，但是了解他可以让我们实现很多有趣的功能。详细介绍可以参考《什么是 JavaScript generator 以及如何使用它们》\[2]和《Generator 函数的含义与用法》\[3]两篇文章。
...
> 这种语法能够让我们以写**同步代码的习惯来编程异步代码**。Generator 实际就是 asyc 函数的语法糖。
>
...
Nodejs 11.0.0.0 以前的版本一次事件循环：执行完一个**主队列中的所有任务**后，再执行微任务队列中的任务
...
![](./image/image_7ZqRZkmAO2.png)

Nodejs 11.0.0 以后的版本一次事件循环和浏览器一样：

执行**完主队列中的一个任务后，立即执行微任务队列中所有任务，然后再执行主任务队列中下一个任务**

![](./image/image_btRQ9qj_RA.png)

![](./image/image_anoG0YjCVX.png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/lP9iauFI73z9nsZj5INI8dx15O06eZQoFlMZOyy9gvXMx6J73hQoQWgy7DBH95eFrDcp0BCIbC7icQeCR4QibNuow/640?wx_fmt=png\&wxfrom=5\&wx_lazy=1\&wx_co=1)

在 Nodejs11 之后版本运行结果

![](https://mmbiz.qpic.cn/mmbiz_png/lP9iauFI73z9nsZj5INI8dx15O06eZQoFCxvVQEHDE6ibiaibdaqL7Iic1seyOeia9xZPFZtPCRv7pgPYDtqckdYFO9g/640?wx_fmt=png\&wxfrom=5\&wx_lazy=1\&wx_co=1)
...
以上问题需要**考虑到异步操作完成时间的不可预知性**，需要考虑不同异步操作对同一个数据所产生的影响。可以**使用锁的思路解决**以上问题。在执行改变页面颜色之前，先判断当前锁的类型是否和任务对应锁的类型相等，如果相当，才执行改变颜色，否则，不执行。

> 给同一个变量；对不同异步操作设置了一个type；异步操作是异步；设置type确实同步马上执行的；锁的思路 有道理
