...
![](./assets/image/image_DE3N51SHby.webp)

1. 什么意思呢？我们删除了 `string数组` 的参数类型。你会发现第二个函数签名飘红报错了。

![](./assets/image/image_4IUV9KuXuZ.webp)

错误信息如下：

![](./assets/image/image_7OwqG6cA-m.webp)

1. 实现签名的返回值同理，不再过多赘述。
2. 最重要的一点来了，你需要在实现签名内非常明确的判断出不同类型的参数所对应的返回值。才能让 `TS` 去实现精确的类型推导。

![](./assets/image/image_UkutChmSdz.webp)
...
![](./assets/image/image_VqNRciAjHs.webp)
...
![](./assets/image/image_Fvg1IJLmKM.webp)
