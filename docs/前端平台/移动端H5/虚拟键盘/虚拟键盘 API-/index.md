...
![](./image/image_9Lk8Ckt1Xw.png)

当输入框处于激活状态时，`checkout` 按钮将位于虚拟键盘下方，因此它被隐藏了。

![](./image/image_CNS2ajGFkB.png)
...
![](./image/image_rb3ET1vtW4.png)

#### 浮动操作按钮

在页面右下角有一个浮动操作按钮。

![](./image/image_n_vkptw3iX.png)
...
![](./image/image_ka0Ao0Sjuu.png)

### 聊天布局

先来看下面的图：

![](./image/image_XVTAxKYiw4.png)
...
![](./image/image_SiafRfqMrr.png)

### LinkedIn 帖子表单和导航

虚拟键盘 API 一个很适用的例子就是 Linkedin 帖子的表单和导航的显示方式。

![](./image/image_OxZIZHQwRS.png)

帖子表单和导航固定在底部。当用户激活输入框时，它看起来像这样：

![](./image/image_D1ToaMWdXG.png)
...
![](./image/image_ojuJoSqlaQ.png)

#### 导航

导航栏的位置是 `bottom: 0`。现在激活的是 `max()` 函数的第一部分。

![](./image/image_y7wZM8mDaH.png)

当虚拟键盘激活时，我们将把导航栏移动到键盘下方。这里的 `100px` 是一个随机数，重点是添加一个大于导航栏高度的值。

![](./image/image_aLTjMowP7m.png)

效果如下：

![](./image/image_cVqnCcqoct.png)
