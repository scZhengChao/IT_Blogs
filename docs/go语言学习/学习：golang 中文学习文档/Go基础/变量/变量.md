...
变量是用于保存一个值的存储位置，允许其存储的值在运行时动态的变化。**除了指针类型，每声明一个变量，都会为其分配一块内存以存储对应类型的值**，前往[参考手册-变量](https://go.dev/ref/spec#Variables "参考手册-变量")以查看更多细节。
...
由于在函数内部**存在未使用的变量会无法通过编译，但有些变量又确实用不到，这个时候就可以使用匿名变量`_`**，使用`_`来表示该变量可以忽略，例如
