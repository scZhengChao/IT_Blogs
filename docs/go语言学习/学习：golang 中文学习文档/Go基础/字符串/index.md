...
在Go中，字符串本质上是一个**不可变的只读的字节数组，也是一片连续的内存空间。**

## [#](https://golang.xiniushu.com/语言入门/语法基础/62.string.html#字面量 "#")字面量

前面提到过字符串有两种字面量表达方式，分为**普通字符串和原生字符串。**
...
因为字符串本质**是字节数组**，所以字符串的访问形式跟数组切片完全一致，例如访问字符串第一个元素
...
**字符串的内容是只读的不可变的，无法修改，**但是**字节切片是可以修改的。**
