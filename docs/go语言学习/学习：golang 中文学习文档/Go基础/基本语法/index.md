...
在Go中，程序是通过将包链接在一起来构建的，也可以理解为**最基本的调用单位是包**，而不是go文件。**包其实就是一个文件夹**，**包内共享所有源文件的变量，常量，函数以及其他类型。**包的命名风格**建议都是小写字母，并且要尽量简短。**
...
下方列出所有的内置关键字，也可以前往[参考手册-标识符](https://go.dev/ref/spec#Identifiers "参考手册-标识符")查看更多细节
...
下面是Go语言中支持的运算符号的优先级排列，也可以前往[参考手册-运算符](https://go.dev/ref/spec#Operators "参考手册-运算符")查看更多细节。
...
字面量，按照计算机科学的术语来讲是用于**表达源代码中一个固定值的符号，也叫字面值**。两个叫法都是一个意思，写了什么东西，值就是什么，值就是“字面意义上“的值。
...
1. 还是花括号，花括号在任何时候都不能够省略，就算是只有一行代码，例如
