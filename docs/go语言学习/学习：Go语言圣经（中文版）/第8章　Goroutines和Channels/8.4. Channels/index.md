...
- [8.4.1. 不带缓存的Channels](#841-不带缓存的Channels)
- [8.4.2. 串联的Channels（Pipeline）](#842-串联的ChannelsPipeline)
- [8.4.3. 单方向的Channel](#843-单方向的Channel)
- [8.4.4. 带缓存的Channels](#844-带缓存的Channels)

如果说goroutine是Go语言程序的并发体的话，那么channels则是它们之间的通信机制。一个channel是一个通信机制，它可以让一个goroutine通过它给另一个goroutine发送值信息。每个channel**都有一个特殊的类型，也就是channels可发送数据的类型。一个可以发送int类型数据的channel一般写为chan int。**
...
**一个基于无缓存Channels的发送操作将导致发送者goroutine阻塞，直到另一个goroutine在相同的Channels上执行接收操作，当发送的值通过Channels成功传输之后，两个goroutine可以继续执行后面的语**句。反之，如果接收操作先发生，那么接收者goroutine也将阻塞，直到有另一个goroutine在相同的Channels上执行发送操作。
...
![](./assets/image/image_Tet3fm2rul.webp)
...
```go title="gopl.io/ch8/pipeline1"
func main() {
...
```go title="gopl.io/ch8/pipeline2"
func main() {
    naturals := make(chan int)
    squares := make(chan int)
...
其实你**并不需要关闭每一个channel。只有当需要告诉接收者goroutine，所有的数据已经全部发送时才需要关闭channel。不管一个channel是否被关闭，当它没有被引用时将会被Go语言的垃圾自动回收器回收。**（**不要将关闭一个打开文件的操作和关闭一个channel操作混淆。对于每个打开的文件，都需要在不使用的时候调用对应的Close方法来关闭文件**。）
...
**因为关闭操作只用于断言不再向channel发送新的数据，**所以只有**在发送者所在的goroutine才会调用close函数，因此对一个只接收的channel调用close将是一个编译错误。**

这是改进的版本，这一次参数使用了单方向channel类型：

```go title="gopl.io/ch8/pipeline3"
func counter(out chan<- int) {
    for x := 0; x < 100; x++ {
        out <- x
    }
...
调用counter（naturals）时，naturals的类型将隐式地从chan int转换成chan<- int。调用printer(squares)也会导致相似的隐式转换，这一次是转换为`<-chan int`类型只接收型的channel。**任何双向channel向单向channel变量的赋值操作都将导致该隐式转换。这里并没有反向转换的语法：也就是不能将一个类似**\*\*`chan<- int`****类型的单向型的channel转换为****`chan int`类型的双向型的\*\*channel。
...
![](./assets/image/image_ghzmLaaKOp.webp)
...
![](./assets/image/image_Vfg4fOXIg1.webp)
...
![](./assets/image/image_TUT18b6kOT.webp)
...
同样，对于内置的len函数，如果传入的是channel，那么将**返回channel内部缓存队列中有效元素的个数。因为在并发程序中该信息会随着接收操作而失效，** 但是它对某些故障诊断和性能优化会有帮助。
...
我们没有太多的空间展示全部细节，[但是gopl.io/ch8/cake包模拟了这个蛋糕店，可以通过不同的参数调整。它还对上面提到的几种场景提供对应的基准测试（§11.4）](http://xn--gopl-4k6fn72j.io/ch8/cake包模拟了这个蛋糕店，可以通过不同的参数调整。它还对上面提到的几种场景提供对应的基准测试（§11.4） "但是gopl.io/ch8/cake包模拟了这个蛋糕店，可以通过不同的参数调整。它还对上面提到的几种场景提供对应的基准测试（§11.4）") 。
