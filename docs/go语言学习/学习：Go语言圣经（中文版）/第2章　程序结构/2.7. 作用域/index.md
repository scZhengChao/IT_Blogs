...
在函数**中词法域可以深度嵌套，因此内部的一个声明可能屏蔽外部的声明**。还有许多语法块是if或for等控制流语句构造的。下面的代码有三个不同的变量x，因为它们是定义在不同的词法域（这个例子只是为了演示作用域规则，但不是好的编程风格）。
...
1. **同一作用域内**：`:=`不是 “重新声明”，而是**赋值**（但有严格规则）；
2. **不同作用域内**：`:=`是**重新声明（变量遮蔽 / Shadowing）**，会创建一个新的同名变量。
